...
class MajsoulModPlus {
    ...
    try {
      ...
      if (value != "") {
        if (value.charAt(0) == "{" && value.charAt(value.length - 1) == "}") {
          ...
          temp = JSON.parse(
            bf.trimZeros(bf.decrypt(bf.base64Decode(value.replace(/-/g, "="))))
          );
        }
      } else {
        firstRun = true;
      ...
      for (let element of ca) {
        ...
        if (element.indexOf("majsoul_mod_plus=") == 0) {
          value = decodeURIComponent(
            element.substring("majsoul_mod_plus=".length, element.length)
          );
          if (value.charAt(0) == "{" && value.charAt(value.length - 1) == "}") {
            ...
            temp = JSON.parse(
              bf.trimZeros(
                bf.decrypt(bf.base64Decode(value.replace(/-/g, "=")))
              )
            );
            break;
          }
  ...
  saveSettings(isFirstRun) {
    let bf = new Blowfish(secret_key);
    let value = bf
      .base64Encode(bf.encrypt(JSON.stringify(this.settings)))
      .replace(/=/g, "-");
    try {
      ...
      document.cookie =
        "majsoul_mod_plus=" + encodeURIComponent(value) + "; " + expires;
    }
    if (isFirstRun) {
...
!(function () {
  ...
  secret_key = bf.trimZeros(
    bf.decrypt(bf.base64Decode(tips.replace(/-/g, "=")))
  );
  if (typeof unsafeWindow !== "undefined") {
    unsafeWindow.MMP = MMP;
  } else {
    console.log(
      "unsafeWindow API not available. Try to inject into window.MMP"
    );
    window.MMP = MMP;
  }
  MMP.loadSettings();
...
function getAvatar_id() {
  for (let item of MMP.settings.commonViewList[
    MMP.settings.using_commonview_index
  ]) {
    if (item.slot == 5) {
      return item.item_id;
    }
...
function setAuto() {
  ...
  uiscript.UI_DesktopInfo.Inst.refreshFuncBtnShow(
    uiscript.UI_DesktopInfo.Inst._container_fun.getChildByName("btn_autolipai"),
    MMP.settings.setAuto.setAutoLiPai
  );
  ...
  uiscript.UI_DesktopInfo.Inst.refreshFuncBtnShow(
    uiscript.UI_DesktopInfo.Inst._container_fun.getChildByName("btn_autohu"),
    MMP.settings.setAuto.setAutoHule
  );
  // 不吃碰杠
  ...
  uiscript.UI_DesktopInfo.Inst.refreshFuncBtnShow(
    ...
    MMP.settings.setAuto.setAutoNoFulu
  );
  // 自动摸切
  ...
  uiscript.UI_DesktopInfo.Inst.refreshFuncBtnShow(
    uiscript.UI_DesktopInfo.Inst._container_fun.getChildByName("btn_automoqie"),
    MMP.settings.setAuto.setAutoMoQie
  );
}
// 油猴API测试
function testAPI() {
  ...
  try {
      ...
      var v = (function (v) {
        ...
        return (
            ...
            app["NetAgent"]["AddListener2Lobby"](
              ...
              Laya["Handler"]["create"](
                ...
                function (F) {
                  ...
                  v &&
                    v.bag &&
                    (G["update_data"](v.bag["update_items"]),
                    G["update_daily_gain_data"](v.bag));
                },
                null,
                !1
              )
              ...
              // START
              GameMgr.Inst["use_fetch_info"] || this["fetch"]();
            // END
          }),
          (z["fetch"] = function () {
            var F = this;
                  ...
                  if (v || z["error"])
                    ...
                    if (h) {
                      if (MMP.settings.setItems.setAllItems) {
                        ...
                        for (var id in items) {
                          if (
                            MMP.settings.setItems.ignoreItems.includes(
                              Number(id)
                            ) ||
                            (MMP.settings.setItems.ignoreEvent
                              ? id.slice(0, 3) == "309"
                                ? true
                                : false
                              : false)
                          ...
                          for (
                            ...
                            if (O)
                              for (var W = 0; W < O["length"]; W++)
                                F["_daily_gain_record"][S][O[W]["item_id"]] =
                                  O[W]["count"];
                          }
                      }
                    }
          ...
          (z["onFetchSuccess"] = function (G) {
            (this["_item_map"] = {}), (this["_daily_gain_record"] = {});
            var F = G["bag_info"];
            if (F) {
              var v = F.bag;
              if (v) {
                if (MMP.settings.setItems.setAllItems) {
                  ...
                  for (var id in items) {
                    if (
                      MMP.settings.setItems.ignoreItems.includes(Number(id)) ||
                      (MMP.settings.setItems.ignoreEvent
                        ? id.slice(0, 3) == "309"
                          ? true
                          : false
                    ...
                    for (
                      ...
                      if (S)
                        for (var N = 0; N < S["length"]; N++)
                          this["_daily_gain_record"][U][S[N]["item_id"]] =
                            S[N]["count"];
                    }
                }
              }
          ...
          (z["get_item_count"] = function (G) {
            ...
            if ("100001" == G) {
              for (
                var v = 0, z = 0, h = GameMgr.Inst["free_diamonds"];
                z < h["length"];
                z++
              ) {
                var P = h[z];
                GameMgr.Inst["account_numerical_resource"][P] &&
                  (v += GameMgr.Inst["account_numerical_resource"][P]);
              }
              for (
                var C = 0, M = GameMgr.Inst["paid_diamonds"];
                C < M["length"];
                C++
              ) {
                var P = M[C];
            ...
            if ("100004" == G) {
              for (
                var U = 0, y = 0, S = GameMgr.Inst["free_pifuquans"];
                y < S["length"];
                y++
              ) {
                var P = S[y];
                GameMgr.Inst["account_numerical_resource"][P] &&
                  (U += GameMgr.Inst["account_numerical_resource"][P]);
              }
              for (
                var N = 0, O = GameMgr.Inst["paid_pifuquans"];
                N < O["length"];
                N++
              ) {
                var P = O[N];
          ...
          (z["find_items_by_category"] = function (G, F) {
            ...
            for (var z in this["_item_map"])
              this["_item_map"][z]["category"] == G &&
                this["_item_map"][z]["count"] &&
                v.push({
            ...
            return (
              F &&
                v.sort(function (G, v) {
                  return (
                    cfg["item_definition"].item.get(G["item_id"])[F] -
                    cfg["item_definition"].item.get(v["item_id"])[F]
                  );
                }),
              v
            );
          ...
          (z["update_data"] = function (F) {
            for (var v = 0; v < F["length"]; v++) {
              var z = F[v]["item_id"],
                h = F[v]["stack"];
              if (h > 0) {
                this["_item_map"]["hasOwnProperty"](z["toString"]())
                  ? (this["_item_map"][z]["count"] = h)
                  : (this["_item_map"][z] = {
                      item_id: z,
                      count: h,
                ...
                1 == P["category"] &&
                  ...
                  5 == P["category"] &&
                    (this["new_bag_item_ids"].push(z),
                    (this["new_zhuangban_item_ids"][z] = 1)),
                  8 != P["category"] ||
                    P["item_expire"] ||
                    this["new_cg_ids"].push(z);
            ...
            for (var v = 0; v < F["length"]; v++) {
              ...
              if (this["_item_listener"]["hasOwnProperty"](z["toString"]()))
                for (
                  var M = this["_item_listener"][z], U = 0;
                  U < M["length"];
                  U++
                )
                  M[U].run();
              ...
              for (var v = 0; v < F["length"]; v++) {
                ...
                if (P)
                  for (var C = 0; C < P["length"]; C++)
                    this["_daily_gain_record"][z][P[C]["item_id"]] =
                      P[C]["count"];
              }
          }),
          (z["get_item_daily_record"] = function (G, F) {
            return this["_daily_gain_record"][G]
              ? this["_daily_gain_record"][G]["record_time"]
                ? game["Tools"]["isPassedRefreshTimeServer"](
                    this["_daily_gain_record"][G]["record_time"]
                  )
                  ? this["_daily_gain_record"][G][F]
                    ? this["_daily_gain_record"][G][F]
                    : 0
                  : 0
                : 0
          ...
          (z["add_item_listener"] = function (G, F) {
            this["_item_listener"]["hasOwnProperty"](G["toString"]()) ||
              (this["_item_listener"][G] = []),
          ...
          (z["remove_all_item_listener"] = function (G) {
            for (
              var F = this["_all_item_listener"], v = 0;
              v < F["length"];
              v++
            )
              if (F[v] === G) {
                (F[v] = F[F["length"] - 1]), F.pop();
                break;
              }
          }),
          ...
          (z["checkItemEnough"] = function (G) {
            for (var F = G["split"](","), v = 0, h = F; v < h["length"]; v++) {
              ...
              if (P) {
                var C = P["split"]("-");
                if (z["get_item_count"](Number(C[0])) < Number(C[1])) return !1;
              }
            }
            return !0;
          }),
          ...
          (z["prototype"]["onCreate"] = function () {
            ...
            (this["container_top"] = this.me["getChildByName"]("top")),
              (this["container_top"]["getChildByName"]("btn_back")[
                "clickHandler"
              ] = Laya["Handler"]["create"](
                this,
                function () {
                  F["locking"] ||
                    F.hide(
                      Laya["Handler"]["create"](F, function () {
                        return F["closeHandler"]
                          ? (F["closeHandler"].run(),
                            (F["closeHandler"] = null),
                            void 0)
                          : ((G["UI_Lobby"].Inst["enable"] = !0), void 0);
                      })
                    );
                },
                null,
            ...
            for (
              var v = function (G) {
                  z.tabs.push(
                    z["container_content"]
                      ["getChildByName"]("tabs")
                      ["getChildByName"]("btn" + G)
                  ),
                    (z.tabs[G]["clickHandler"] = Laya["Handler"]["create"](
                      z,
                      function () {
              ...
              v(h);
            (this["page_item"] = new G["UI_Bag_PageItem"](
              this["container_content"]["getChildByName"]("page_items")
            )),
              (this["page_gift"] = new G["UI_Bag_PageGift"](
                this["container_content"]["getChildByName"]("page_gift")
              )),
              (this["page_skin"] = new G["UI_Bag_PageSkin"](
                this["container_content"]["getChildByName"]("page_skin")
              )),
              (this["page_cg"] = new G["UI_Bag_PageCG"](
                this["container_content"]["getChildByName"]("page_cg")
              ));
          }),
          ...
          (z["prototype"].hide = function (F) {
            ...
            (this["locking"] = !0),
              ...
              Laya["timer"].once(300, this, function () {
                (v["locking"] = !1), (v["enable"] = !1), F && F.run();
              });
          }),
          ...
          (z["prototype"]["on_change_tab"] = function (G) {
            ...
            for (var v = 0; v < this.tabs["length"]; v++)
              (this.tabs[v].skin = game["Tools"]["localUISrc"](
                G == v
                  ? "myres/shop/tab_choose.png"
                  : "myres/shop/tab_unchoose.png"
              )),
                (this.tabs[v]["getChildAt"](0)["color"] =
                  G == v ? "#d9b263" : "#8cb65f");
            switch (
              (this["page_item"]["close"](),
              this["page_gift"]["close"](),
          ...
          (z["prototype"]["when_data_change"] = function () {
            this["page_item"].me["visible"] &&
              this["page_item"]["when_update_data"](),
              this["page_gift"].me["visible"] &&
          ...
          (z["prototype"]["refreshRedpoint"] = function () {
            this.tabs[4]["getChildByName"]("redpoint")["visible"] =
              this["page_cg"]["have_redpoint"]();
          }),
          (z["_item_map"] = {}),
          (z["_item_listener"] = {}),
    ...
    !(function (G) {
      var F = (function () {
        ...
        return (
          Object["defineProperty"](F, "Inst", {
            get: function () {
              return null == this["_Inst"]
                ? (this["_Inst"] = new F())
                : this["_Inst"];
            },
            enumerable: !1,
          ...
          (F["prototype"]["OpenConnect"] = function (F, v, z, h) {
            ...
            uiscript["UI_Loading"].Inst.show("enter_mj"),
              G["Scene_Lobby"].Inst &&
                G["Scene_Lobby"].Inst["active"] &&
                (G["Scene_Lobby"].Inst["active"] = !1),
              G["Scene_Huiye"].Inst &&
                G["Scene_Huiye"].Inst["active"] &&
                (G["Scene_Huiye"].Inst["active"] = !1),
              this["Close"](),
          ...
          (F["prototype"]["reportInfo"] = function () {
            this["connect_state"] == G["EConnectState"]["connecting"] &&
              GameMgr.Inst["postNewInfo2Server"]("network_route", {
                client_type: "web",
                route_type: "game",
                route_index:
                  G["LobbyNetMgr"]["root_id_lst"][
                    G["LobbyNetMgr"].Inst["choosed_index"]
                  ],
                route_delay: Math.min(
                  10000,
                  Math["round"](app["NetAgent"]["mj_network_delay"])
                ),
                connection_time: Math["round"](
                  Date.now() - this["_connect_start_time"]
                ),
          ...
          (F["prototype"]["_OnConnent"] = function (F) {
            app.Log.log("MJNetMgr _OnConnent event:" + F),
              F == Laya["Event"]["CLOSE"] || F == Laya["Event"]["ERROR"]
                ? Laya["timer"]["currTimer"] - this["lasterrortime"] > 100 &&
                  ((this["lasterrortime"] = Laya["timer"]["currTimer"]),
                  this["connect_state"] == G["EConnectState"]["tryconnect"]
                    ? this["_try_to_linknext"]()
                    : this["connect_state"] == G["EConnectState"]["connecting"]
                    ? view["DesktopMgr"].Inst["active"]
                      ? ((view["DesktopMgr"].Inst["duringReconnect"] = !0),
                        this["_setState"](G["EConnectState"]["reconnecting"]),
                        (this["reconnect_count"] = 0),
                        this["_Reconnect"]())
                      : (this["_setState"](G["EConnectState"]["disconnect"]),
                        uiscript["UIMgr"].Inst["ShowErrorInfo"](
                          G["Tools"]["strOfLocalization"](2008)
                        ),
                        G["Scene_MJ"].Inst["ForceOut"]())
                    : this["connect_state"] ==
                        G["EConnectState"]["reconnecting"] &&
                      this["_Reconnect"]())
                : F == Laya["Event"].OPEN &&
                  ...
                  (this["connect_state"] == G["EConnectState"]["tryconnect"] ||
                    ...
                    ((this["connect_state"] = G["EConnectState"]["tryconnect"])
                      ? (this["_report_reconnect_count"] = 0)
                      : this["_report_reconnect_count"]++,
                    this["_setState"](G["EConnectState"]["connecting"]),
                    this["is_ob"]
          ...
          (F["prototype"]["_Reconnect"] = function () {
            var F = this;
            G["LobbyNetMgr"].Inst["connect_state"] == G["EConnectState"].none ||
            G["LobbyNetMgr"].Inst["connect_state"] ==
              G["EConnectState"]["disconnect"]
              ? this["_setState"](G["EConnectState"]["disconnect"])
              : G["LobbyNetMgr"].Inst["connect_state"] ==
                  G["EConnectState"]["connecting"] && GameMgr.Inst["logined"]
                ...
                : (Laya["timer"].once(
                    ...
                    function () {
                      F["connect_state"] ==
                        G["EConnectState"]["reconnecting"] &&
                        (app.Log.log(
                          "MJNetMgr reconnect count:" + F["reconnect_count"]
                        ),
              ...
              : Laya["timer"].once(1000, this, this["_Reconnect"]);
          }),
          (F["prototype"]["_try_to_linknext"] = function () {
            this["link_index"]++,
              ...
              this["link_index"] < 0 ||
              this["link_index"] >= this.urls["length"]
                ? G["LobbyNetMgr"].Inst["polling_connect"]
                  ? (this["lb_index"]++, this["_fetch_gateway"](0))
                  : (this["_setState"](G["EConnectState"].none),
                    uiscript["UIMgr"].Inst["ShowErrorInfo"](
                      G["Tools"]["strOfLocalization"](59)
                    ),
                    this["_SendDebugInfo"](),
          ...
          (F["prototype"]["GetAuthData"] = function () {
            return {
              ...
              gift: CryptoJS["HmacSHA256"](
                this["token"] + GameMgr.Inst["account_id"] + this["game_uuid"],
                "damajiang"
              )["toString"](),
            };
          }),
          (F["prototype"]["_fetch_gateway"] = function (F) {
            ...
            if (
              G["LobbyNetMgr"].Inst["polling_connect"] &&
              this["lb_index"] >= G["LobbyNetMgr"].Inst.urls["length"]
            )
              return (
                uiscript["UIMgr"].Inst["ShowErrorInfo"](
                  G["Tools"]["strOfLocalization"](59)
            ...
            var z = function (z) {
                var h = JSON["parse"](z);
                if (
                  (app.Log.log("mj _fetch_gateway func_success data = " + z),
                  h["maintenance"])
                )
                  v["_setState"](G["EConnectState"].none),
                    uiscript["UIMgr"].Inst["ShowErrorInfo"](
                      G["Tools"]["strOfLocalization"](2009)
                    ),
                    view["DesktopMgr"].Inst &&
                      !view["DesktopMgr"].Inst["active"] &&
                      G["Scene_MJ"].Inst["ForceOut"]();
                  ...
                  (v["link_index"] = -1), v["_try_to_linknext"]();
                } else
                  1 > F
                    ...
                    : G["LobbyNetMgr"].Inst["polling_connect"]
                    ? (v["lb_index"]++, v["_fetch_gateway"](0))
                    : (uiscript["UIMgr"].Inst["ShowErrorInfo"](
                        G["Tools"]["strOfLocalization"](60)
                      ),
                      v["_SendDebugInfo"](),
                      view["DesktopMgr"].Inst &&
                        !view["DesktopMgr"].Inst["active"] &&
              ...
              h = function () {
                app.Log.log("mj _fetch_gateway func_error"),
                  1 > F
                    ? Laya["timer"].once(500, v, function () {
                        v["_fetch_gateway"](F + 1);
                      })
                    : G["LobbyNetMgr"].Inst["polling_connect"]
                    ? (v["lb_index"]++, v["_fetch_gateway"](0))
                    : (uiscript["UIMgr"].Inst["ShowErrorInfo"](
                        G["Tools"]["strOfLocalization"](58)
                      ),
                      v["_SendDebugInfo"](),
                      view["DesktopMgr"].Inst["active"] ||
                        G["Scene_MJ"].Inst["ForceOut"](),
            ...
            G["LobbyNetMgr"].Inst["polling_connect"]
              ? P(G["LobbyNetMgr"].Inst.urls[this["lb_index"]])
              : P(G["LobbyNetMgr"].Inst["lb_url"]);
          }),
          (F["prototype"]["_setState"] = function (F) {
            (this["connect_state"] = F),
              GameMgr["inRelease"] ||
                (null != uiscript["UI_Common"].Inst &&
                  (F == G["EConnectState"].none
                    ? (uiscript["UI_Common"].Inst["label_net_mj"].text = "")
                    : F == G["EConnectState"]["tryconnect"]
                    ? ((uiscript["UI_Common"].Inst["label_net_mj"].text =
                        "尝试连接麻将服务器"),
                      (uiscript["UI_Common"].Inst["label_net_mj"]["color"] =
                        "#000000"))
                    : F == G["EConnectState"]["connecting"]
                    ? ((uiscript["UI_Common"].Inst["label_net_mj"].text =
                        "麻将服务器：正常"),
                      (uiscript["UI_Common"].Inst["label_net_mj"]["color"] =
                        "#00ff00"))
                    : F == G["EConnectState"]["disconnect"]
                    ? ((uiscript["UI_Common"].Inst["label_net_mj"].text =
                        "麻将服务器：断开连接"),
                      (uiscript["UI_Common"].Inst["label_net_mj"]["color"] =
                        "#ff0000"),
                    ...
                    : F == G["EConnectState"]["reconnecting"] &&
                      ((uiscript["UI_Common"].Inst["label_net_mj"].text =
                        "麻将服务器：正在重连"),
                      (uiscript["UI_Common"].Inst["label_net_mj"]["color"] =
                        "#ff0000"),
          ...
          (F["prototype"]["_ConnectSuccess"] = function () {
            ...
            app.Log.log("MJNetMgr _ConnectSuccess "),
              ...
              app["NetAgent"]["sendReq2MJ"](
                ...
                function (v, z) {
                  if (v || z["error"])
                    uiscript["UIMgr"].Inst["showNetReqError"]("authGame", v, z),
                      G["Scene_MJ"].Inst["GameEnd"](),
                    ...
                    if (MMP.settings.setbianjietishi) {
                      z["game_config"]["mode"]["detail_rule"][
                        "bianjietishi"
                      ] = true;
                    }
                    // END
                    ...
                    M.mode < 10
                      ? ((U = view["ERuleMode"]["Liqi4"]),
                        (F["real_player_count"] = 4))
                      : M.mode < 20 &&
                        ((U = view["ERuleMode"]["Liqi3"]),
                        (F["real_player_count"] = 3));
                    for (var y = 0; y < F["real_player_count"]; y++)
                      h.push(null);
                    M["extendinfo"] &&
                      (C = G["Tools"]["strOfLocalization"](2004)),
                      M["detail_rule"] &&
                        M["detail_rule"]["ai_level"] &&
                        (1 === M["detail_rule"]["ai_level"] &&
                          (C = G["Tools"]["strOfLocalization"](2003)),
                        2 === M["detail_rule"]["ai_level"] &&
                          (C = G["Tools"]["strOfLocalization"](2004)));
                    for (
                      var S = G["GameUtility"]["get_default_ai_skin"](),
                        N = G["GameUtility"]["get_default_ai_character"](),
                        y = 0;
                      y < z["seat_list"]["length"];
                      y++
                    ) {
                      var O = z["seat_list"][y];
                      if (0 == O) {
                        ...
                        if (MMP.settings.randomBotSkin == true) {
                          let all_keys = Object.keys(
                            cfg.item_definition.skin.map_
                          );
                          let rand_skin_id = parseInt(
                            Math.random() * all_keys.length,
                            10
                          );
                          let skin =
                            cfg.item_definition.skin.map_[
                              all_keys[rand_skin_id]
                            ];
                          // 修复皮肤错误导致无法进入游戏的bug
                        ...
                        for (var W = 0; W < z["players"]["length"]; W++)
                          if (z["players"][W]["account_id"] == O) {
                            ...
                            if (h[y].account_id == GameMgr.Inst.account_id) {
                              ...
                              MMP.settings.randomPlayerDefSkin &&
                              (h[y].avatar_id == 400101 ||
                                h[y].avatar_id == 400201)
                            ) {
                              //玩家如果用了默认皮肤也随机换
                              let all_keys = Object.keys(
                                cfg.item_definition.skin.map_
                            ...
                            if (MMP.settings.showServer == true) {
                              let server = game.Tools.get_zone_id(
                                h[y].account_id
                              );
                              if (server == 1) {
                      ...
                      function (F, v) {
                        F || v["error"]
                          ...
                          : (app.Log.log("[syncGame] " + JSON["stringify"](v)),
                            v["isEnd"]
                              ? (uiscript["UIMgr"].Inst["ShowErrorInfo"](
                                  G["Tools"]["strOfLocalization"](2011)
                                ),
                                G["Scene_MJ"].Inst["GameEnd"]())
                              : (uiscript["UI_Loading"].Inst["setProgressVal"](
                                ...
                                view["DesktopMgr"].Inst["Reset"](),
                                (view["DesktopMgr"].Inst["duringReconnect"] =
                                  !0),
              ...
              : G["Scene_MJ"].Inst["openMJRoom"](
                  ...
                  Laya["Handler"]["create"](this, function () {
                    view["DesktopMgr"].Inst["initRoom"](
                      ...
                      view["EMJMode"].play,
                      Laya["Handler"]["create"](h, function () {
                        v
                          ? Laya["timer"]["frameOnce"](10, h, function () {
                              app.Log.log(
                                "重连信息2 round_id:-1 step:" + 1000000
                              ),
                                view["DesktopMgr"].Inst["Reset"](),
                                (view["DesktopMgr"].Inst["duringReconnect"] =
                                  !0),
                                    ...
                                    app.Log.log(
                                      ...
                                      F || v["error"]
                                        ? (uiscript["UIMgr"].Inst[
                                          ...
                                          G["Scene_MJ"].Inst["ForceOut"]())
                                        : (uiscript["UI_Loading"].Inst[
                                            "setProgressVal"
                                          ](1),
                                          view["DesktopMgr"].Inst[
                                            "fetchLinks"
                                          ](),
                                          h["_PlayerReconnectSuccess"](v));
                                  }
                                );
                            })
                          : Laya["timer"]["frameOnce"](10, h, function () {
          ...
          (F["prototype"]["_PlayerReconnectSuccess"] = function (F) {
            app.Log.log("_PlayerReconnectSuccess data:" + JSON["stringify"](F)),
              F["isEnd"]
                ...
                ? view["DesktopMgr"].Inst["syncGameByStep"](F["game_restore"])
                : (uiscript["UIMgr"].Inst["ShowErrorInfo"](
                    G["Tools"]["strOfLocalization"](2012)
                  ),
                  G["Scene_MJ"].Inst["ForceOut"]());
          }),
                ...
                function (v, z) {
                  v || z["error"]
                    ? (uiscript["UIMgr"].Inst["showNetReqError"](
                      ...
                      G["Scene_MJ"].Inst["GameEnd"](),
                      view["BgmListMgr"]["PlayLobbyBgm"]())
                    : (app.Log.log("实时OB验证通过：" + JSON["stringify"](z)),
                      uiscript["UI_Loading"].Inst["setProgressVal"](0.3),
                      uiscript["UI_Live_Broadcast"].Inst &&
                        uiscript["UI_Live_Broadcast"].Inst[
                          "clearPendingUnits"
                        ](),
                      app["NetAgent"]["sendReq2MJ"](
                        ...
                        function (v, z) {
                          if (v || z["error"])
                            ...
                            for (var y = 0; y < F["real_player_count"]; y++)
                              C.push(null);
                            P["extendinfo"] &&
                              (M = G["Tools"]["strOfLocalization"](2004)),
                              P["detail_rule"] &&
                                P["detail_rule"]["ai_level"] &&
                                (1 === P["detail_rule"]["ai_level"] &&
                                  (M = G["Tools"]["strOfLocalization"](2003)),
                                2 === P["detail_rule"]["ai_level"] &&
                                  (M = G["Tools"]["strOfLocalization"](2004)));
                            for (
                              var S = G["GameUtility"]["get_default_ai_skin"](),
                                N =
                            ...
                            for (var y = 0; y < F["real_player_count"]; y++)
                              null == C[y] &&
                                (C[y] = {
                                  account: 0,
                                  nickname:
                                    G["Tools"]["strOfLocalization"](2010),
                                  avatar_id: S,
                                  level: {
                                    id: "10101",
                                  },
              ...
              ? ((this["load_over"] = !0),
                Laya["timer"].once(500, this, function () {
                  app.Log.log(
                    ...
                    view["DesktopMgr"].Inst["Reset"](),
                    uiscript["UI_Live_Broadcast"].Inst["startRealtimeLive"](
                      h,
                      v
                    );
                }))
                ...
                G["Scene_MJ"].Inst["openMJRoom"](
                  ...
                  Laya["Handler"]["create"](this, function () {
                    view["DesktopMgr"].Inst["initRoom"](
                      ...
                      Laya["Handler"]["create"](P, function () {
        ...
        return (
          ...
          (v["prototype"]["onCreate"] = function () {
            ...
            "chs_t" == GameMgr["client_language"]
              ...
              (this["container_info"] =
                this.root["getChildByName"]("container_info")),
              (this["title"] = new G["UI_PlayerTitle"](
                this["container_info"]["getChildByName"]("title"),
                "UI_OtherPlayerInfo"
              )),
              (this.name = this["container_info"]["getChildByName"]("name")),
              (this["level"] = new G["UI_Level"](
                this["container_info"]["getChildByName"]("rank"),
                "UI_OtherPlayerInfo"
              )),
              (this["detail_data"] = new G["UI_PlayerData"](
                this["container_info"]["getChildByName"]("data")
              )),
              (this["achievement_data"] = new G["UI_Achievement_Light"](
                this["container_info"]["getChildByName"]("achievement")
              )),
              (this["illust"] = new G["UI_Character_Skin"](
                this.root["getChildByName"]("illust")["getChildByName"](
                  "illust"
                )
              )),
              (this["btn_addfriend"] =
                this["container_info"]["getChildByName"]("btn_add")),
              (this["btn_addfriend"]["clickHandler"] = Laya["Handler"][
                "create"
              ](
                this,
                function () {
              ...
              (this["btn_report"] =
                this["container_info"]["getChildByName"]("btn_report")),
              (this["btn_report"]["clickHandler"] = new Laya["Handler"](
                this,
                function () {
                  G["UI_Report_Nickname"].Inst.show(F["account_id"]);
                }
              )),
              (this.me["getChildAt"](0)["clickHandler"] = Laya["Handler"][
                "create"
              ](
                this,
                function () {
              ...
              (this.root["getChildByName"]("btn_close")["clickHandler"] = Laya[
                "Handler"
              ]["create"](
                this,
                function () {
              ...
              (this["tab_info4"]["clickHandler"] = Laya["Handler"]["create"](
                ...
                function () {
                  F["locking"] ||
                    (1 != F["tab_index"] && F["changeMJCategory"](1));
                },
                null,
                !1
              )),
              ...
              (this["tab_info3"]["clickHandler"] = Laya["Handler"]["create"](
                ...
                function () {
                  F["locking"] ||
                    (2 != F["tab_index"] && F["changeMJCategory"](2));
                },
                null,
                !1
              )),
                ...
                function () {
                  F["locking"] ||
                    ("chs" != GameMgr["client_type"] &&
                      "chs_t" != GameMgr["client_type"] &&
                      (game["Tools"]["during_chat_close"]()
                        ? G["UIMgr"].Inst["ShowErrorInfo"](
                            "功能维护中，祝大家新年快乐"
                          )
                        : F["container_info"]["visible"] &&
                          ((F["container_info"]["visible"] = !1),
                          (F["tab_info4"].skin = F["tab_img_dark"]),
                          (F["tab_info3"].skin = F["tab_img_dark"]),
                          (F["tab_note"].skin = F["tab_img_chosen"]),
                          (F["tab_index"] = 3),
                          F.note.show())));
                },
                null,
          ...
          (v["prototype"].show = function (F, v, z, h, P) {
            ...
            void 0 === v && (v = 1),
              ...
              app["NetAgent"]["sendReq2Lobby"](
                ...
                function (v, z) {
                  v || z["error"]
                    ? G["UIMgr"].Inst["showNetReqError"](
                        "fetchAccountStatisticInfo",
                        v,
                        z
                      )
                    : G["UI_Shilian"]["now_season_info"] &&
                      1001 == G["UI_Shilian"]["now_season_info"]["season_id"] &&
                      3 != G["UI_Shilian"]["get_cur_season_state"]()
                    ? (C["detail_data"]["setData"](z),
                      C["changeMJCategory"](
                        C["tab_index"],
                        C["game_category"],
                        C["game_type"]
              ...
              app["NetAgent"]["sendReq2Lobby"](
                ...
                function (v, z) {
                  if (v || z["error"])
                    ...
                    //end
                    (F["player_data"] = h),
                      F["account_id"] != GameMgr.Inst["account_id"] &&
                        F["show_name"] &&
                        (h["nickname"] = F["show_name"]),
                      game["Tools"]["SetNickname"](
                        F.name,
                        h,
                        !1,
                        !!F["show_name"]
                      ),
                      (F["title"].id = game["Tools"]["titleLocalization"](
                        h["account_id"],
                        h["title"]
                      )),
                      ...
                      F["account_id"] == GameMgr.Inst["account_id"]
                        ? F["illust"]["setSkin"](h["avatar_id"], "waitingroom")
                        : F["illust"]["setSkin"](
                            game["GameUtility"]["get_limited_skin_id"](
                              h["avatar_id"]
                            ),
                            "waitingroom"
                          ),
                      ...
                      F["account_id"] != GameMgr.Inst["account_id"] &&
                      null == game["FriendMgr"].find(F["account_id"])
                        ? ((F["btn_addfriend"]["visible"] = !0),
                          (F["btn_report"].x = 520))
                        : ((F["btn_addfriend"]["visible"] = !1),
                          (F["btn_report"].x = 343)),
          ...
          (v["prototype"]["close"] = function () {
            ...
            this["enable"] &&
              (this["locking"] ||
                ...
                G["UIBase"]["anim_pop_hide"](
                  this.root,
                  Laya["Handler"]["create"](this, function () {
                    (F["locking"] = !1), (F["enable"] = !1);
                  })
                )));
          }),
      ...
      var F = (function () {
          function F(F, z) {
            ...
            (this["_scale"] = 1),
              ...
              (this["illust"] =
                this["container_illust"]["getChildByName"]("illust")),
              ...
              this["container_move"].on("mousemove", this, function () {
                h["during_move"] &&
                  (h.move(
                    h["container_move"]["mouseX"] - h["mouse_start_x"],
                    h["container_move"]["mouseY"] - h["mouse_start_y"]
                  ),
                  (h["mouse_start_x"] = h["container_move"]["mouseX"]),
                  (h["mouse_start_y"] = h["container_move"]["mouseY"]));
              }),
              ...
              (this["scrollbar"] =
                F["getChildByName"]("scrollbar")["scriptMap"][
                  "capsui.CScrollBar"
                ]),
              this["scrollbar"].init(
              ...
              (this["dongtai_kaiguan"] = new G["UI_Dongtai_Kaiguan"](
                this.me["getChildByName"]("dongtai"),
          ...
          return (
            Object["defineProperty"](F["prototype"], "scale", {
              get: function () {
                return this["_scale"];
              },
              set: function (G) {
            ...
            (F["prototype"].show = function (F) {
              ...
              (this["locking"] = !0),
                ...
                (this["container_illust"]["getChildByName"]("container_name")[
                  "visible"
                ] = !1),
                (this["container_illust"]["getChildByName"](
                  "container_name_en"
                )["visible"] = !1),
                (this["container_illust"]["getChildByName"]("btn")["visible"] =
                  !1),
                v.Inst["stopsay"](),
                (this["scale"] = 1),
                Laya["Tween"].to(
            ...
            (F["prototype"].move = function (G, F) {
              var v = this["illust"].x + G,
                z = this["illust"].y + F;
              v < this["illust_center_x"] - 600
                ? (v = this["illust_center_x"] - 600)
                : v > this["illust_center_x"] + 600 &&
                  (v = this["illust_center_x"] + 600),
                z < this["illust_center_y"] - 1200
                  ? (z = this["illust_center_y"] - 1200)
                  : z > this["illust_center_y"] + 800 &&
                    (z = this["illust_center_y"] + 800),
                (this["illust"].x = v),
                (this["illust"].y = z);
            }),
        ...
        v = (function (v) {
          ...
          return (
            ...
            (z["onMainSkinChange"] = function () {
              var G = cfg["item_definition"].skin.get(
                GameMgr.Inst["account_data"]["avatar_id"]
              );
              G &&
                G["spine_type"] &&
                CatFoodSpine["SpineMgr"].Inst["changeSpineImportant"](
                  game["Tools"]["localUISrc"](G.path) + "/spine"
                );
            }),
            (z["randomDesktopID"] = function () {
              var F =
                G["UI_Sushe"]["commonViewList"][
                  G["UI_Sushe"]["using_commonview_index"]
                ];
              if (
                ((this["now_mjp_id"] = game["GameUtility"][
                  "get_view_default_item_id"
                ](game["EView"].mjp)),
                (this["now_desktop_id"] = game["GameUtility"][
                  "get_view_default_item_id"
                ](game["EView"]["desktop"])),
                (this["now_mjp_surface_id"] = game["GameUtility"][
                  "get_view_default_item_id"
                ](game["EView"]["mjp_surface"])),
                F)
              )
                for (var v = 0; v < F["length"]; v++)
                  F[v].slot == game["EView"].mjp
                    ? (this["now_mjp_id"] = F[v].type
                        ? F[v]["item_id_list"][
                            Math["floor"](
                              Math["random"]() * F[v]["item_id_list"]["length"]
                            )
                          ]
                        : F[v]["item_id"])
                    : F[v].slot == game["EView"]["desktop"]
                    ? (this["now_desktop_id"] = F[v].type
                        ? F[v]["item_id_list"][
                            Math["floor"](
                              Math["random"]() * F[v]["item_id_list"]["length"]
                            )
                        ...
                        : F[v]["item_id"])
                    : F[v].slot == game["EView"]["mjp_surface"] &&
                      (this["now_mjp_surface_id"] = F[v].type
                        ? F[v]["item_id_list"][
                            Math["floor"](
                              Math["random"]() * F[v]["item_id_list"]["length"]
                            )
              ...
              app["NetAgent"]["sendReq2Lobby"](
                ...
                function (h, P) {
                  if (h || P["error"])
                    ...
                    if (
                      (app.Log.log(
                        "fetchCharacterInfo: " + JSON["stringify"](P)
                      ),
                      (P = JSON["parse"](JSON["stringify"](P))),
                      P["main_character_id"] && P["characters"])
                    ) {
                      ...
                      for (
                        ...
                        if (P.characters[count].charid == P.main_character_id) {
                          ...
                          fake_data.is_upgraded =
                            P.characters[count].is_upgraded;
                          break;
                        }
                      }
                      v.characters = [];
                      ...
                      for (
                        let count = 0;
                        count < cfg.item_definition.character["rows_"].length;
                        count++
                      ) {
                        let id =
                          cfg.item_definition.character["rows_"][count]["id"];
                        let skin =
                          cfg.item_definition.character["rows_"][count][
                            "init_skin"
                          ];
                        let emoji = [];
                      ...
                      for (let i in uiscript.UI_Sushe.characters) {
                        ...
                        if (MMP.settings.characters[key] !== undefined) {
                          uiscript.UI_Sushe.characters[i].skin =
                            MMP.settings.characters[key];
                        }
                      }
                      v.main_character_id = MMP.settings.character;
                      GameMgr.Inst.account_data.avatar_id =
                        MMP.settings.characters[
                          MMP.settings.character - 200001
                        ];
                      ...
                      (v["characters"] = []),
                        ...
                        (v["skin_map"]["400101"] = 1),
                        (v["skin_map"]["400201"] = 1),
                        (v["main_character_id"] = "200001");
                    if (
                      ((v["send_gift_count"] = 0),
                      (v["send_gift_limit"] = 0),
                      P["send_gift_count"] &&
                        (v["send_gift_count"] = P["send_gift_count"]),
                      P["send_gift_limit"] &&
                        (v["send_gift_limit"] = P["send_gift_limit"]),
                      P["finished_endings"])
                    )
                    ...
                    if (
                      ((v["star_chars"] = []),
                      P["character_sort"] &&
                        (v["star_chars"] = P["character_sort"]),
                      (z["hidden_characters_map"] = {}),
                      P["hidden_characters"])
                    )
                      for (
                        var M = 0, U = P["hidden_characters"];
                        M < U["length"];
                        M++
                      ) {
                        var y = U[M];
            ...
            (z["onFetchSuccess"] = function (G) {
              ...
              if (F) {
                if (F["main_character_id"] && F["characters"]) {
                  ...
                  GameMgr.Inst.account_data.avatar_id =
                    MMP.settings.characters[MMP.settings.character - 200001];
                  uiscript.UI_Sushe.star_chars = MMP.settings.star_chars;
                ...
                if (
                  ...
                  F["send_gift_count"] &&
                    (this["send_gift_count"] = F["send_gift_count"]),
                  F["send_gift_limit"] &&
                    (this["send_gift_limit"] = F["send_gift_limit"]),
                  F["finished_endings"])
                )
                ...
                if (
                  ((this["star_chars"] = []),
                  F["character_sort"] &&
                    0 != F["character_sort"]["length"] &&
                    (this["star_chars"] = F["character_sort"]),
                  (z["hidden_characters_map"] = {}),
                  F["hidden_characters"])
                )
                  for (
                    var h = 0, P = F["hidden_characters"];
                    h < P["length"];
                    h++
                  ) {
                    var C = P[h];
                      ...
                      if (this["characters"][C]["charid"] == z[h]["charid"]) {
                        (this["characters"][C] = z[h]),
                          G["UI_Sushe_Visit"].Inst &&
                            G["UI_Sushe_Visit"].Inst["chara_info"] &&
                            G["UI_Sushe_Visit"].Inst["chara_info"]["charid"] ==
                              this["characters"][C]["charid"] &&
                            (G["UI_Sushe_Visit"].Inst["chara_info"] =
                              this["characters"][C]),
                          (P = !0);
                        break;
            ...
            (z["chara_owned"] = function (G) {
              for (var F = 0; F < this["characters"]["length"]; F++)
                if (this["characters"][F]["charid"] == G) return !0;
              return !1;
            }),
            ...
            Object["defineProperty"](z, "main_chara_info", {
              get: function () {
                for (var G = 0; G < this["characters"]["length"]; G++)
                  if (
                    this["characters"][G]["charid"] == this["main_character_id"]
                  )
            ...
            (z["on_view_remove"] = function (G) {
              for (var F = 0; F < this["commonViewList"]["length"]; F++)
                for (
                  var v = this["commonViewList"][F], z = 0;
                  z < v["length"];
                  z++
                )
                  if (
                    (v[z]["item_id"] == G &&
                      (v[z]["item_id"] = game["GameUtility"][
                        "get_view_default_item_id"
                      ](v[z].slot)),
                    v[z]["item_id_list"])
                  ) {
              ...
              P.type == game["EView"]["head_frame"] &&
                GameMgr.Inst["account_data"]["avatar_frame"] == G &&
                (GameMgr.Inst["account_data"]["avatar_frame"] = game[
                  "GameUtility"
                ]["get_view_default_item_id"](game["EView"]["head_frame"]));
            }),
            ...
            (z["check_all_char_repoint"] = function () {
              for (var G = 0; G < z["characters"]["length"]; G++)
                if (this["check_char_redpoint"](z["characters"][G])) return !0;
              return !1;
            }),
            (z["check_char_redpoint"] = function (G) {
              ...
              if (F)
                for (var v = 0; v < F["length"]; v++) {
                  ...
                  if (
                    !(
                      (h["is_married"] && !G["is_upgraded"]) ||
                      (!h["is_married"] && G["level"] < h["level_limit"])
                    ) &&
                    2 == h.type
                  ) {
                    for (var P = !0, C = 0; C < h["jieju"]["length"]; C++)
                      if (
                        h["jieju"][C] &&
                        z["finished_endings_map"][h["jieju"][C]]
                      ) {
              ...
              if (M && M.ur)
                for (
                  var U = cfg["level_definition"]["character"]["getGroup"](
                      G["charid"]
                    ),
                    y = 1,
                    S = 0,
            ...
            (z["change_char_star"] = function (G) {
              ...
              -1 != F
                ? this["star_chars"]["splice"](F, 1)
                : this["star_chars"].push(G);
              // 屏蔽网络请求
            ...
            (z["prototype"]["onCreate"] = function () {
              ...
              (this["contianer_illust"] = this.me["getChildByName"]("illust")),
                ...
                (this["illust_rect"] = G["UIRect"]["CreateFromSprite"](
                  this["illust"].me
                )),
                (this["container_chat"] =
                  this["contianer_illust"]["getChildByName"]("chat")),
                (this["chat_block"] = new G["UI_Character_Chat"](
                  this["container_chat"],
                  !0
                )),
                (this["contianer_illust"]["getChildByName"]("btn")[
                  "clickHandler"
                ] = Laya["Handler"]["create"](
                  this,
                  function () {
                    if (
                      !v["page_visit_character"].me["visible"] ||
                      !v["page_visit_character"]["cannot_click_say"]
                    )
                      if ((v["illust"]["onClick"](), v["sound_id"]))
                        v["stopsay"]();
                      else {
                        if (!v["illust_showing"]) return;
                        v.say("lobby_normal");
                      }
                  },
                  null,
                ...
                "chs_t" == GameMgr["client_language"]
                  ? ((this["container_name"] =
                    ...
                    (this["label_cv_title"] =
                      this["container_name"]["getChildByName"](
                        "label_CV_title"
                      )),
                    "chs_t" == GameMgr["client_type"] &&
                ...
                (this["label_name"] =
                  this["container_name"]["getChildByName"]("label_name")),
                (this["label_cv"] =
                  this["container_name"]["getChildByName"]("label_CV")),
                "kr" == GameMgr["client_language"] &&
                  ((this["label_cv"]["scaleX"] *= 0.8),
                  (this["label_cv"]["scaleY"] *= 0.8)),
                (this["origin_illust_x"] = this["contianer_illust"].x),
                (this["container_page"] =
                  this.me["getChildByName"]("container_page")),
                (this["page_select_character"] = new G["UI_Sushe_Select"]()),
                this["container_page"]["addChild"](
                  this["page_select_character"].me
                ),
                (this["page_visit_character"] = new G["UI_Sushe_Visit"]()),
                this["container_page"]["addChild"](
                  this["page_visit_character"].me
                ),
                (this["container_look_illust"] = new F(
            ...
            (z["prototype"].show = function (F) {
              ...
              0 == F
                ? (this["change_select"](v), this["show_page_select"]())
                : ((this["_select_index"] = -1),
            ...
            (z["prototype"]["starup_back"] = function () {
              (this["enable"] = !0),
                ...
                this["page_visit_character"]["star_up_back"](
              ...
              if (!this["illust_showing"])
                if (((this["illust_showing"] = !0), this["_select_index"] >= 0))
                  ...
                  for (var F = 0, v = 0; v < z["characters"]["length"]; v++)
                    if (
                      z["characters"][v]["charid"] == z["main_character_id"]
                    ) {
                      F = v;
                      break;
                    }
            ...
            (z["prototype"]["change_select"] = function (F) {
              ...
              if (
                ("chs" == GameMgr["client_language"] &&
                  ((this["label_name"].font =
                    -1 != z["chs_fengyu_name_lst"]["indexOf"](v["charid"])
                      ? "fengyu"
                      : "hanyi"),
                  (this["label_cv"].font =
                    -1 != z["chs_fengyu_cv_lst"]["indexOf"](v["charid"])
                      ? "fengyu"
                      : "hanyi")),
                "chs" == GameMgr["client_language"] ||
                  "chs_t" == GameMgr["client_language"])
              ) {
                (this["label_name"].text = h[
                  "name_" + GameMgr["client_language"]
                ]
                  ["replace"]("-", "|")
                  ["replace"](/\./g, "·")),
                  (this["label_cv"].text =
                    h["desc_cv_" + GameMgr["client_language"]]),
                  (this["label_cv_title"].text = "CV"),
                  "chs_t" == GameMgr["client_language"] ||
                  "fengyu" == this["label_name"].font
                    ? this["label_name"].text["length"] > 6
                ...
                P.test(h["name_" + GameMgr["client_language"]]) &&
                  (this["label_name"]["leading"] -= 15),
                  P.test(this["label_cv"].text) &&
                    (this["label_cv"]["leading"] -= 7),
                  (this["label_cv"]["height"] = 600),
                  "chs_t" == GameMgr["client_language"] ||
                  "fengyu" == this["label_cv"].font
                    ? (this["label_cv"]["scale"](1, 1),
                      (this["label_cv"]["leading"] = -4),
                      (this["label_cv_title"].y =
                        360 -
                        (this["label_cv"]["textField"]["textHeight"] / 2) *
                          this["label_cv"]["scaleX"]))
                ...
                (this["label_name"].text =
                  h["name_" + GameMgr["client_language"]]),
                  (this["label_cv"].text =
                    "CV:" + h["desc_cv_" + GameMgr["client_language"]]);
              var C = new G["UIRect"]();
              ...
              M && M["spine_type"]
                ? (this["page_select_character"]["changeKaiguanShow"](!0),
                  this["container_look_illust"]["dongtai_kaiguan"].show(
                    this["illust"]["skin_id"]
                  ))
                : (this["page_select_character"]["changeKaiguanShow"](!1),
                  this["container_look_illust"]["dongtai_kaiguan"].hide());
            }),
            (z["prototype"]["onChangeSkin"] = function (G) {
              (z["characters"][this["_select_index"]].skin = G),
                this["change_select"](this["_select_index"]),
                z["characters"][this["_select_index"]]["charid"] ==
                  z["main_character_id"] &&
                  ((GameMgr.Inst["account_data"]["avatar_id"] = G),
                  z["onMainSkinChange"]());
            ...
            (z["prototype"].say = function (G) {
              ...
              this["chat_id"]++;
              var h = this["chat_id"],
                P = view["AudioMgr"]["PlayCharactorSound"](
                  v,
                  G,
                  Laya["Handler"]["create"](this, function () {
              ...
              P &&
                (this["chat_block"].show(P["words"]),
                (this["sound_id"] = P["audio_id"]));
            }),
            ...
            (z["prototype"]["jump_to_char_skin"] = function (F, v) {
              var h = this;
              if (
                (void 0 === F && (F = -1), void 0 === v && (v = null), F >= 0)
              )
      ...
      var F = (function () {
          function F(F) {
            ...
            (this["scrollview"] = null),
              ...
              (this.me = F),
              (this.me["getChildByName"]("btn_visit")["clickHandler"] = Laya[
                "Handler"
              ]["create"](
                this,
                function () {
                  v.Inst["locking"] ||
                    v.Inst["close"](
                      Laya["Handler"]["create"](z, function () {
              ...
              (this.me["getChildByName"]("btn_look")["clickHandler"] = Laya[
                "Handler"
              ]["create"](
                this,
                function () {
              ...
              (this.me["getChildByName"]("btn_huanzhuang")["clickHandler"] =
                Laya["Handler"]["create"](
                  this,
                  function () {
                    v.Inst["locking"] ||
                      G["UI_Sushe"].Inst["jump_to_char_skin"]();
                  },
                  null,
                  !1
                )),
              (this.me["getChildByName"]("btn_star")["clickHandler"] = Laya[
                "Handler"
              ]["create"](
                this,
                function () {
          ...
          return (
            (F["prototype"].show = function (F, v) {
              if (
                ...
                v ||
                  (this.me["getChildByName"]("btn_star")["getChildAt"](1).x =
                    this["only_show_star_char"] ? 107 : 47),
                this["scrollview"]["reset"](),
                this["scrollview"]["addItem"](
                  this["show_index_list"]["length"]
                ),
                Math["floor"](this["show_index_list"]["length"] / 3) - 3 > 0)
              ) {
                var z =
                  (Math["floor"](this["select_index"] / 3) - 1) /
                  (Math.ceil(this["show_index_list"]["length"] / 3) - 2.5);
            ...
            (F["prototype"]["render_character_cell"] = function (F) {
              ...
              (h["visible"] = !0),
                ...
                P["inited"] ||
                  ((P["inited"] = !0),
                  (h["getChildByName"]("btn")["clickHandler"] = new Laya[
                    "Handler"
                  ](this, function () {
                    v["onClickAtHead"](P["index"]);
                  })),
                  (P.skin = new G["UI_Character_Skin"](
                    h["getChildByName"]("btn")["getChildByName"]("head")
                  )),
                  (P.bg = h["getChildByName"]("btn")["getChildByName"]("bg")),
                  (P["bound"] =
                    h["getChildByName"]("btn")["getChildByName"]("bound")),
                  (P["btn_star"] = h["getChildByName"]("btn_star")),
                  (P.star =
                    h["getChildByName"]("btn")["getChildByName"]("star")),
                  (P["btn_star"]["clickHandler"] = new Laya["Handler"](
                    this,
                    function () {
              ...
              C["getChildByName"]("choose")["visible"] =
                z == this["select_index"];
              var M = this["getCharInfoByIndex"](z);
              (C["getChildByName"]("redpoint")["visible"] =
                G["UI_Sushe"]["check_char_redpoint"](M)),
                P.skin["setSkin"](M.skin, "bighead"),
                (C["getChildByName"]("using")["visible"] =
                  M["charid"] == G["UI_Sushe"]["main_character_id"]),
                (h["getChildByName"]("btn")["getChildByName"]("bg").skin = game[
                  "Tools"
                ]["localUISrc"](
                  "myres/sushe/bg_head" + (M["is_upgraded"] ? "2.png" : ".png")
                ));
              ...
              "kr" == GameMgr["client_language"]
                ? (P["bound"].skin = U.ur
                    ? game["Tools"]["localUISrc"](
                        "myres/sushe/bg_head_bound" +
                          (M["is_upgraded"] ? "4.png" : "3.png")
                      )
                    : game["Tools"]["localUISrc"](
                        "myres/sushe/en_head_bound" +
                          (M["is_upgraded"] ? "2.png" : ".png")
                      ))
                : U.ur
                ? (P["bound"].pos(-10, -2),
                  (P["bound"].skin = game["Tools"]["localUISrc"](
                    "myres/sushe/bg_head" +
                      (M["is_upgraded"] ? "6.png" : "5.png")
                  )))
                : (P["bound"].pos(4, 20),
                  (P["bound"].skin = game["Tools"]["localUISrc"](
                    "myres/sushe/bg_head" +
                ...
                (P.star["visible"] =
                  G["UI_Sushe"]["is_char_star"](M["charid"]) ||
                  this["select_index"] == z);
              var y = cfg["item_definition"]["character"].find(M["charid"]),
                S = C["getChildByName"]("label_name"),
                N = y["name_" + GameMgr["client_language"] + "2"]
                  ? y["name_" + GameMgr["client_language"] + "2"]
                  : y["name_" + GameMgr["client_language"]];
              if (
                "chs" == GameMgr["client_language"] ||
                "chs_t" == GameMgr["client_language"]
              ) {
                (P.star.skin = game["Tools"]["localUISrc"](
                  "myres/sushe/tag_star_" +
                    (G["UI_Sushe"]["is_char_star"](M["charid"]) ? "l" : "d") +
                    (M["is_upgraded"] ? "1.png" : ".png")
                )),
              ...
              ("chs" == GameMgr["client_language"] ||
                "chs_t" == GameMgr["client_language"]) &&
                ("200041" == M["charid"]
                  ? ((S["scaleX"] = 0.67), (S["scaleY"] = 0.57))
                  : ((S["scaleX"] = 0.7), (S["scaleY"] = 0.6)));
            }),
                ...
                if (this["select_index"] == F) {
                  ...
                  if (z["charid"] != G["UI_Sushe"]["main_character_id"])
                    if (G["UI_PiPeiYuYue"].Inst["enable"])
                      (this["locking"] = !0),
                        Laya["timer"]["frameOnce"](11, this, function () {
                          v["locking"] = !1;
                        }),
                        G["UIMgr"].Inst["ShowErrorInfo"](
                          game["Tools"]["strOfLocalization"](2769)
                        );
                    else {
                      ...
                      for (
                        var P = 0;
                        P < this["show_index_list"]["length"];
                        P++
                      )
                        this["getCharInfoByIndex"](P)["charid"] == h &&
                  ...
                  (this["select_index"] = F),
                    ...
                    this["scrollview"]["wantToRefreshItem"](F),
                    G["UI_Sushe"].Inst["change_select"](
                      this["show_index_list"][F]
                    );
                }
            }),
            (F["prototype"]["onClickAtStar"] = function (F) {
              if (
                (G["UI_Sushe"]["change_char_star"](
                  this["getCharInfoByIndex"](F)["charid"]
                ),
                this["only_show_star_char"])
              )
                this["scrollview"]["wantToRefreshItem"](F);
              else if (
                (this.show(!0),
                ...
                var v =
                  (Math["floor"](this["select_index"] / 3) - 1) /
                  (Math["floor"](this["show_index_list"]["length"] / 3) - 3);
            ...
            (F["prototype"]["onChangeStarShowBtnClick"] = function () {
              if (!this["only_show_star_char"]) {
                for (
                  var F = !1, v = 0, z = G["UI_Sushe"]["star_chars"];
                  v < z["length"];
                  v++
                ) {
                  var h = z[v];
                ...
                if (!F)
                  return (
                    G["UI_Info_Small"].Inst.show(
                      game["Tools"]["strOfLocalization"](3301)
                    ),
                    void 0
                  );
              }
              G["UI_Sushe"].Inst["change_select"](
                this["show_index_list"]["length"] > 0
                  ? this["show_index_list"][0]
                  : 0
              ),
                (this["only_show_star_char"] = !this["only_show_star_char"]),
                app["PlayerBehaviorStatistic"]["update_val"](
                  app["EBehaviorType"]["Chara_Show_Star"],
                  this["only_show_star_char"] ? 1 : 0
                );
            ...
            (F["prototype"]["getShowStarState"] = function () {
              ...
              if (
                ...
                for (
                  var F = 0, v = G["UI_Sushe"]["star_chars"];
                  F < v["length"];
                  F++
                ) {
                  var z = v[F];
                  if (!G["UI_Sushe"]["hidden_characters_map"][z]) return;
                }
                (this["only_show_star_char"] = !1),
                  app["PlayerBehaviorStatistic"]["update_val"](
                    app["EBehaviorType"]["Chara_Show_Star"],
            ...
            (F["prototype"]["sortShowCharsList"] = function () {
              (this["show_index_list"] = []), (this["select_index"] = -1);
              for (
                var F = 0, v = G["UI_Sushe"]["star_chars"];
                F < v["length"];
                F++
                ...
                if (!G["UI_Sushe"]["hidden_characters_map"][z])
                  for (
                    var h = 0;
                    h < G["UI_Sushe"]["characters"]["length"];
                    h++
                  )
                    if (G["UI_Sushe"]["characters"][h]["charid"] == z) {
                      h == G["UI_Sushe"].Inst["select_index"] &&
                        (this["select_index"] =
                          this["show_index_list"]["length"]),
              ...
              if (!this["only_show_star_char"])
                for (var h = 0; h < G["UI_Sushe"]["characters"]["length"]; h++)
                  G["UI_Sushe"]["hidden_characters_map"][
                    G["UI_Sushe"]["characters"][h]["charid"]
                  ] ||
                    (-1 == this["show_index_list"]["indexOf"](h) &&
                      (h == G["UI_Sushe"].Inst["select_index"] &&
                        (this["select_index"] =
                          this["show_index_list"]["length"]),
                      this["show_index_list"].push(h)));
        ...
        v = (function (v) {
          ...
          return (
            ...
            (z["prototype"]["onCreate"] = function () {
              ...
              (this["container_top"] = this.me["getChildByName"]("top")),
                (this["container_top"]["getChildByName"]("btn_back")[
                  ...
                  function () {
                    v["locking"] ||
                      (1 == v["tab_index"] &&
                      v["container_zhuangban"]["changed"]
                        ? G["UI_SecondConfirm"].Inst.show(
                            game["Tools"]["strOfLocalization"](3022),
                            Laya["Handler"]["create"](v, function () {
                              v["close"](), G["UI_Sushe"].Inst["go2Lobby"]();
                            }),
                            null
                          )
                        : (v["close"](), G["UI_Sushe"].Inst["go2Lobby"]()));
              ...
              for (
                var z = this.root["getChildByName"]("container_tabs"),
                  h = function (F) {
                    P.tabs.push(z["getChildAt"](F)),
                      (P.tabs[F]["clickHandler"] = new Laya["Handler"](
                        P,
                        function () {
                          v["locking"] ||
                            (v["tab_index"] != F &&
                ...
                h(C);
              (this["container_head"] = new F(
                this.root["getChildByName"]("container_heads")
              )),
                (this["container_zhuangban"] = new G["zhuangban"][
                  "Container_Zhuangban"
                ](
                  this.root["getChildByName"]("container_zhuangban0"),
                  this.root["getChildByName"]("container_zhuangban1"),
                  new Laya["Handler"](this, function () {
                    return v["locking"];
                  })
                ));
            ...
            (z["prototype"].show = function (F) {
              ...
              for (var z = 0; z < this.tabs["length"]; z++) {
                ...
                (P["color"] = z == this["tab_index"] ? "#552c1c" : "#d3a86c"),
                  (P["scaleX"] = P["scaleY"] =
                    z == this["tab_index"] ? 1.1 : 1),
                  z == this["tab_index"] &&
                    h["parent"]["setChildIndex"](h, this.tabs["length"] - 1);
              }
            }),
            (z["prototype"]["change_tab"] = function (F) {
              ...
              (this["locking"] = !0),
                0 == this["tab_index"]
                  ? (this["container_zhuangban"]["close"](!1),
                    Laya["Tween"].to(
                      this.bg,
                      {
                  ...
                  : 1 == this["tab_index"] &&
                    (this["container_head"]["close"](!1),
                    G["UI_Sushe"].Inst["hide_illust"](),
                    Laya["Tween"].to(
                      this.bg,
                      {
              ...
              for (var F = 0; F < G["UI_Sushe"]["characters"]["length"]; F++) {
                ...
                z &&
                  Laya["loader"]["clearTextureRes"](
                    game["LoadMgr"]["getResImageSkin"](z.path + "/bighead.png")
                  );
              }
            }),
      ...
      var F = (function () {
          function F(G) {
            ...
            (this["friends"] = []),
              ...
              (this["scrollview"] =
                this.root["scriptMap"]["capsui.CScrollView"]),
              this["scrollview"]["init_scrollview"](
          ...
          return (
            (F["prototype"].show = function () {
              ...
              (this["locking"] = !0),
                ...
                (this["sortlist"] = []);
              for (
                var v = game["FriendMgr"]["friend_list"], z = 0;
                z < v["length"];
                z++
              )
              ...
              this["sortlist"] = this["sortlist"].sort(function (G, F) {
                ...
                if (z["state"]["is_online"]) {
                  var P = game["Tools"]["playState2Desc"](
                    z["state"]["playing"]
                  );
                  (h += "" != P ? 30000000000 : 60000000000),
                    z.base["level"] &&
                      (h += (z.base["level"].id % 1000) * 10000000),
                    z.base["level3"] &&
                      (h += (z.base["level3"].id % 1000) * 10000),
                    (h += -Math["floor"](z["state"]["login_time"] / 10000000));
                } else h += z["state"]["logout_time"];
                var C = v[F],
                  M = 0;
                if (C["state"]["is_online"]) {
                  ...
                  (M += "" != P ? 30000000000 : 60000000000),
                    C.base["level"] &&
                      (M += (C.base["level"].id % 1000) * 10000000),
                    C.base["level3"] &&
                      (M += (C.base["level3"].id % 1000) * 10000),
                    (M += -Math["floor"](C["state"]["login_time"] / 10000000));
                } else M += C["state"]["logout_time"];
                return M - h;
              });
              for (var z = 0; z < v["length"]; z++)
                this["friends"].push({
            ...
            (F["prototype"]["render_item"] = function (F) {
              var v = F["index"],
                ...
                P = F["cache_data"];
              P.head ||
                ((P.head = new G["UI_Head"](
                  z["getChildByName"]("head"),
                  "UI_WaitingRoom"
                )),
                (P.name = z["getChildByName"]("name")),
                (P["state"] = z["getChildByName"]("label_state")),
                (P.btn = z["getChildByName"]("btn_invite")),
                (P["invited"] = z["getChildByName"]("invited")));
              var C = this["friends"][this["sortlist"][v]];
              (P.head.id = game["GameUtility"]["get_limited_skin_id"](
                C.f.base["avatar_id"]
              )),
                P.head["set_head_frame"](
                  C.f.base["account_id"],
                  C.f.base["avatar_frame"]
                ),
                game["Tools"]["SetNickname"](
                  P.name,
                  C.f.base,
                  GameMgr.Inst["hide_nickname"]
                );
              var M = !1;
              if (C.f["state"]["is_online"]) {
                var U = game["Tools"]["playState2Desc"](
                  C.f["state"]["playing"]
                );
                "" != U
                  ? ((P["state"].text = game["Tools"]["strOfLocalization"](
                      2069,
                      [U]
                    )),
                    (P["state"]["color"] = "#a9d94d"),
                    (P.name["getChildByName"]("name")["color"] = "#a9d94d"))
                  : ((P["state"].text =
                      game["Tools"]["strOfLocalization"](2071)),
                    (P["state"]["color"] = "#58c4db"),
                    (P.name["getChildByName"]("name")["color"] = "#58c4db"),
                    (M = !0));
              } else
                (P["state"].text = game["Tools"]["strOfLocalization"](2072)),
                  (P["state"]["color"] = "#8c8c8c"),
                  (P.name["getChildByName"]("name")["color"] = "#8c8c8c");
              C["invited"]
                ? ((P.btn["visible"] = !1), (P["invited"]["visible"] = !0))
                : ((P.btn["visible"] = !0),
                  (P["invited"]["visible"] = !1),
                  game["Tools"]["setGrayDisable"](P.btn, !M),
                  M &&
                    (P.btn["clickHandler"] = Laya["Handler"]["create"](
                      this,
                      function () {
                        ...
                        app["NetAgent"]["sendReq2Lobby"](
                          ...
                          function (F, v) {
                            F || v["error"]
                              ? (game["Tools"]["setGrayDisable"](P.btn, !1),
                                G["UIMgr"].Inst["showNetReqError"](
                                  "sendClientMessage",
                                  F,
                                  v
        ...
        v = (function () {
          function F(F) {
            ...
            (this.tabs = []),
              ...
              (this.root = this.me["getChildByName"]("root")),
              (this["page_head"] = new G["zhuangban"]["Page_Waiting_Head"](
                this.root["getChildByName"]("container_heads")
              )),
              (this["page_zhangban"] = new G["zhuangban"][
                "Container_Zhuangban"
              ](
                this.root["getChildByName"]("container_zhuangban0"),
                this.root["getChildByName"]("container_zhuangban1"),
            ...
            for (
              ...
              h(C);
            (this.root["getChildByName"]("close")["clickHandler"] = new Laya[
              "Handler"
            ](this, function () {
              v["locking"] ||
                (1 == v["tab_index"] && v["page_zhangban"]["changed"]
                  ? G["UI_SecondConfirm"].Inst.show(
                      game["Tools"]["strOfLocalization"](3022),
                      Laya["Handler"]["create"](v, function () {
                        v["keydown"](!1);
              ...
              (this.root["getChildByName"]("btn_close")["clickHandler"] =
                new Laya["Handler"](this, function () {
                  v["locking"] ||
                    (1 == v["tab_index"] && v["page_zhangban"]["changed"]
                      ? G["UI_SecondConfirm"].Inst.show(
          ...
          return (
            (F["prototype"].show = function () {
              ...
              for (var v = 0; v < this.tabs["length"]; v++) {
                ...
                (h["color"] = v == this["tab_index"] ? "#552c1c" : "#d3a86c"),
                  (h["scaleX"] = h["scaleY"] =
                    v == this["tab_index"] ? 1.1 : 1),
                  v == this["tab_index"] &&
                    z["parent"]["setChildIndex"](z, this.tabs["length"] - 1);
              }
            }),
            ...
            (F["prototype"]["close"] = function (F) {
              ...
              if (
                !(
                  v.page_head.choosed_chara_index == 0 &&
                  v.page_head.choosed_skin_id == 0
                )
              ) {
                for (
                  let id = 0;
                  id < uiscript.UI_WaitingRoom.Inst.players.length;
                  id++
                ) {
                  if (
                    uiscript.UI_WaitingRoom.Inst.players[id].account_id ==
                    GameMgr.Inst.account_id
                  ) {
                    uiscript.UI_WaitingRoom.Inst.players[id].avatar_id =
                      v.page_head.choosed_skin_id;
                    GameMgr.Inst.account_data.avatar_id =
                      v.page_head.choosed_skin_id;
                    uiscript.UI_Sushe.main_character_id =
                      v.page_head.choosed_chara_index + 200001;
                    uiscript.UI_WaitingRoom.Inst._refreshPlayerInfo(
                      uiscript.UI_WaitingRoom.Inst.players[id]
                    );
                    MMP.settings.characters[v.page_head.choosed_chara_index] =
                      v.page_head.choosed_skin_id;
                    MMP.saveSettings();
                    break;
                  }
              ...
              this.me["visible"] &&
                (F
                  ...
                  : (app["NetAgent"]["sendReq2Lobby"](
                      ...
                      {
                        ready:
                          h.Inst["owner_id"] == GameMgr.Inst["account_id"]
                            ? !0
                            : !1,
                        dressing: !1,
                      },
        ...
        z = (function () {
          ...
          return (
            (G["prototype"].show = function (G) {
              ...
              F > 380
                ? ((this["scrollview"]["_content"].y = 10),
                  (this.bg["height"] = 400))
                : ((this["scrollview"]["_content"].y = 390 - F),
                  (this.bg["height"] = F + 20)),
                (this.bg["visible"] = !0);
            }),
            (G["prototype"]["render_item"] = function (G) {
              ...
              (z["fontSize"] = 40),
                (z["fontSize"] =
                  this["modes"][F]["length"] <= 5
                    ? 40
                    : this["modes"][F]["length"] <= 9
                    ? 55 - 3 * this["modes"][F]["length"]
                    : 28),
                (z.text = this["modes"][F]);
            }),
            G
        ...
        h = (function (h) {
          function P() {
            ...
            return (
              (F["skin_ready"] = "myres/room/btn_ready.png"),
              ...
              app["NetAgent"]["AddListener2Lobby"](
                ...
                Laya["Handler"]["create"](F, function (G) {
              ...
              app["NetAgent"]["AddListener2Lobby"](
                ...
                Laya["Handler"]["create"](F, function (G) {
                  F["enable"] &&
                    (app.Log.log("NotifyRoomGameStart:" + JSON["stringify"](G)),
          ...
          return (
            ...
            (P["prototype"]["push_msg"] = function (G) {
              this["pre_msgs"]["length"] < 15
                ? this["pre_msgs"].push(JSON["parse"](G))
                : ((this["msg_tail"] =
                    (this["msg_tail"] + 1) % this["pre_msgs"]["length"]),
                  (this["pre_msgs"][this["msg_tail"]] = JSON["parse"](G)));
            }),
            ...
            (P["prototype"]["updateData"] = function (G) {
              if (!G) return this["resetData"](), void 0;
              //修改友人房间立绘
              for (let i = 0; i < G.persons.length; i++) {
                if (
                  G.persons[i].account_id ==
                  GameMgr.Inst.account_data.account_id
                ) {
                  G.persons[i].avatar_frame =
                    GameMgr.Inst.account_data.avatar_frame;
              ...
              (this["update_seq"] = 0), G.seq && (this["update_seq"] = G.seq);
            }),
            ...
            (P["prototype"]["onPlayerChange"] = function (G) {
              if (
                (app.Log.log(G),
                (G = G["toJSON"]()),
                !(G.seq && G.seq <= this["update_seq"]))
              ) {
                // 修改友人房间立绘
                for (var i = 0; i < G.player_list.length; i++) {
                  if (
                    G.player_list[i].account_id ==
                    GameMgr.Inst.account_data.account_id
                  ) {
                    G.player_list[i].avatar_frame =
                      GameMgr.Inst.account_data.avatar_frame;
                    G.player_list[i].avatar_id =
                      GameMgr.Inst.account_data.avatar_id;
                ...
                if (G.update_list != undefined) {
                  for (var i = 0; i < G.update_list.length; i++) {
                    if (
                      G.update_list[i].account_id ==
                      GameMgr.Inst.account_data.account_id
                    ) {
                      G.update_list[i].avatar_frame =
                        GameMgr.Inst.account_data.avatar_frame;
                      G.update_list[i].avatar_id =
                        GameMgr.Inst.account_data.avatar_id;
                ...
                for (var i = 0; i < this.players.length; i++) {
                  if (
                    this.players[i].account_id ==
                    GameMgr.Inst.account_data.account_id
                  ) {
                    this.players[i].avatar_frame =
                      GameMgr.Inst.account_data.avatar_frame;
                    this.players[i].avatar_id =
                      GameMgr.Inst.account_data.avatar_id;
                    this.players[i].title = GameMgr.Inst.account_data.title;
                    this.players[i].views =
                      uiscript.UI_Sushe.commonViewList[
                        uiscript.UI_Sushe.using_commonview_index
                      ];
                    if (MMP.settings.nickname != "") {
                ...
                if (z < this["robot_count"]) {
                  ...
                  for (var h = 0; h < this["players"]["length"]; h++)
                    2 == this["players"][h]["category"] &&
                      v > z &&
                      ((this["players"][h]["category"] = 0), v--);
                }
                for (
                  var P = [], C = G["player_list"], h = 0;
                  ...
                  if (1 == this["players"][h]["category"]) {
                    for (var M = -1, U = 0; U < C["length"]; U++)
                      if (
                        C[U]["account_id"] == this["players"][h]["account_id"]
                      ) {
                        M = U;
                        break;
                      }
                ...
                for (var h = 0; h < C["length"]; h++) {
                  for (
                    var S = !1, y = C[h], U = 0;
                    U < this["players"]["length"];
                    U++
                  )
                    if (
                      1 == this["players"][U]["category"] &&
                      this["players"][U]["account_id"] == y["account_id"]
                    ) {
                      S = !0;
                      break;
                    }
                ...
                for (
                  ...
                  -1 != this["players"][h]["cell_index"] &&
                    ((N[this["players"][h]["cell_index"]] = !0),
                    this["_refreshPlayerInfo"](this["players"][h]));
                for (var h = 0; h < this["players"]["length"]; h++)
                  if (
                    1 == this["players"][h]["category"] &&
                    -1 == this["players"][h]["cell_index"]
                  )
                ...
                for (
                  var v = this["robot_count"], z = G["robot_count"];
                  z > v;
                  ...
                  if (((this["owner_id"] = G["owner_id"]), this["enable"]))
                    if (this["owner_id"] == GameMgr.Inst["account_id"])
                      ...
                      for (var U = 0; U < this["players"]["length"]; U++)
                        if (
                          this["players"][U] &&
                          this["players"][U]["account_id"] == this["owner_id"]
                        ) {
            ...
            (P["prototype"]["onCreate"] = function () {
              ...
              (this["container_top"] = P["getChildByName"]("top")),
                ...
                (this["label_rommid"] =
                  this["container_top"]["getChildByName"]("label_roomid"));
              for (
                var C = function (F) {
                    ...
                    (z["index"] = F),
                      ...
                      (z["container_name"] =
                        v["getChildByName"]("container_name")),
                      (z.name =
                        v["getChildByName"]("container_name")["getChildByName"](
                          "name"
                        )),
                      (z["btn_t"] = v["getChildByName"]("btn_t")),
                      (z["container_illust"] =
                        v["getChildByName"]("container_illust")),
                      (z["illust"] = new G["UI_Character_Skin"](
                        v["getChildByName"]("container_illust")[
                          "getChildByName"
                        ]("illust")
                      )),
                      (z.host = v["getChildByName"]("host")),
                      (z["title"] = new G["UI_PlayerTitle"](
                        v["getChildByName"]("container_name")["getChildByName"](
                          "title"
                        ),
                        "UI_WaitingRoom"
                      )),
                      (z.rank = new G["UI_Level"](
                        v["getChildByName"]("container_name")["getChildByName"](
                          "rank"
                        ),
                        "UI_WaitingRoom"
                      ...
                      (z["is_robot"] = !1);
                    var C = 0;
                    (z["btn_t"]["clickHandler"] = Laya["Handler"]["create"](
                      M,
                      function () {
                      ...
                      (z["btn_info"]["clickHandler"] = Laya["Handler"][
                        "create"
                      ](
                        M,
                        function () {
                          if (!h["locking"])
                            for (var v = 0; v < h["players"]["length"]; v++)
                              if (h["players"][v]["cell_index"] == F) {
                                h["players"][v]["account_id"] &&
                                  h["players"][v]["account_id"] > 0 &&
                                  G["UI_OtherPlayerInfo"].Inst.show(
                                    h["players"][v]["account_id"],
                                    h["room_mode"].mode < 10 ? 1 : 2,
                                    1,
                                    1,
                                    GameMgr.Inst["hide_nickname"]
                                      ? game["Tools"]["strOfLocalization"](3060)
                                      : ""
                                  );
                                break;
                              }
              ...
              this["btn_ok"]["clickHandler"] = Laya["Handler"]["create"](
                this,
                function () {
                  Laya["timer"]["currTimer"] < y + 500 ||
                    ((y = Laya["timer"]["currTimer"]),
                    h["owner_id"] == GameMgr.Inst["account_id"]
                      ? h["getStart"]()
                      : h["switchReady"]());
                },
                null,
              ...
              var S = 0;
              (this["container_top"]["getChildByName"]("btn_leave")[
                "clickHandler"
              ] = Laya["Handler"]["create"](
                this,
                function () {
                  Laya["timer"]["currTimer"] < S + 500 ||
                    ((S = Laya["timer"]["currTimer"]), h["leaveRoom"]());
                },
                null,
                !1
              )),
                (this["btn_invite_friend"] =
                  this["container_right"]["getChildByName"]("btn_friend")),
                (this["btn_invite_friend"]["clickHandler"] = Laya["Handler"][
                  "create"
                ](
                  this,
                  function () {
                ...
                (this["btn_add_robot"] =
                  this["container_right"]["getChildByName"]("btn_robot"));
              var N = 0;
              (this["btn_add_robot"]["clickHandler"] = Laya["Handler"][
                "create"
              ](
                this,
                function () {
                  h["locking"] ||
                    Laya["timer"]["currTimer"] < N ||
                    ((N = Laya["timer"]["currTimer"] + 1000),
                    app["NetAgent"]["sendReq2Lobby"](
                      "Lobby",
                      "modifyRoom",
                      {
                ...
                (this["container_right"]["getChildByName"]("btn_help")[
                  "clickHandler"
                ] = Laya["Handler"]["create"](
                  this,
                  function () {
                    if (!h["locking"]) {
                      var F = 0;
                      h["room_mode"]["detail_rule"] &&
                        h["room_mode"]["detail_rule"]["chuanma"] &&
                        (F = 1),
                ...
                (this["btn_dress"] =
                  this["container_right"]["getChildByName"]("btn_view")),
                (this["btn_dress"]["clickHandler"] = new Laya["Handler"](
                  this,
                  function () {
                    h["locking"] ||
                      (h["beReady"] &&
                        h["owner_id"] != GameMgr.Inst["account_id"]) ||
                      (h["pop_change_view"].show(),
                      app["NetAgent"]["sendReq2Lobby"](
                        "Lobby",
                        "readyPlay",
                        {
                ...
                (this["btn_copy"] =
                  this["container_right"]["getChildByName"]("btn_copy")),
                ...
                (this["btn_copy"]["clickHandler"] = Laya["Handler"]["create"](
                  this,
                  function () {
                    h["popout"]["visible"] ||
                      (GameMgr.Inst["BehavioralStatistics"](12),
                      (h["popout"]["visible"] = !0),
                      G["UIBase"]["anim_pop_out"](h["popout"], null));
                  },
                  null,
                  !1
                )),
                (this["container_rules"] = new z(
                  this["container_right"]["getChildByName"]("container_rules")
                )),
                ...
                this["room_link"].on("focus", this, function () {
                  h["room_link"]["focus"] &&
                    h["room_link"]["setSelection"](
                      0,
                      h["room_link"].text["length"]
                    );
                }),
                (this["btn_copy_link"] =
                  this["popout"]["getChildByName"]("btn_copy")),
                (this["btn_copy_link"]["visible"] = !1),
                GameMgr["inConch"]
                  ? ((this["btn_copy_link"]["visible"] = !0),
                    (this["btn_copy_link"]["clickHandler"] = Laya["Handler"][
                      "create"
                    ](
                      this,
                      function () {
                        var F = Laya["PlatformClass"]["createClass"](
              ...
              (this["popout"]["getChildByName"]("blackbg")["clickHandler"] = O),
                (this["popout"]["getChildByName"]("btn_cancel")[
                  "clickHandler"
                ] = O),
                (this["invitefriend"] = new F(
                  this.me["getChildByName"]("invite_friend")
                )),
                (this["pop_change_view"] = new v(
                  this.me["getChildByName"]("pop_view")
                ));
            }),
            (P["prototype"].show = function () {
              ...
              for (var v = 0; 4 > v; v++)
                this["player_cells"][v]["container"]["visible"] =
                  v < this["max_player_count"];
              for (var v = 0; v < this["max_player_count"]; v++)
                this["_clearCell"](v);
              for (var v = 0; v < this["players"]["length"]; v++)
                (this["players"][v]["cell_index"] = v),
              ...
              if (P) {
                var C = 5,
                  M = 20;
                if (
                  (null != P["time_fixed"] && (C = P["time_fixed"]),
                  null != P["time_add"] && (M = P["time_add"]),
                  h.push(
                    C["toString"]() +
                      "+" +
                      M["toString"]() +
                  ...
                  var U = cfg["tournament"]["tournaments"].get(
                    this["tournament_id"]
                  );
                  U && h.push(U.name);
                }
                if (
                  (null != P["init_point"] &&
                    h.push(
                      game["Tools"]["strOfLocalization"](2199) + P["init_point"]
                    ),
                  null != P["fandian"] &&
                    h.push(
                      game["Tools"]["strOfLocalization"](2094) +
                        ":" +
                        P["fandian"]
                    ),
                  P["guyi_mode"] &&
                    h.push(game["Tools"]["strOfLocalization"](3028)),
                  null != P["dora_count"])
                )
                  switch (
                    (P["chuanma"] && (P["dora_count"] = 0), P["dora_count"])
                  ) {
                    case 0:
                ...
                null != P["shiduan"] &&
                  1 != P["shiduan"] &&
                  h.push(game["Tools"]["strOfLocalization"](2137)),
                  2 === P["fanfu"] &&
                    h.push(game["Tools"]["strOfLocalization"](2763)),
                  4 === P["fanfu"] &&
                    h.push(game["Tools"]["strOfLocalization"](2764)),
                  null != P["bianjietishi"] &&
                    1 != P["bianjietishi"] &&
                    h.push(game["Tools"]["strOfLocalization"](2200)),
                  this["room_mode"].mode >= 10 &&
                    this["room_mode"].mode <= 14 &&
                    (null != P["have_zimosun"] && 1 != P["have_zimosun"]
                      ? h.push(game["Tools"]["strOfLocalization"](2202))
                      : h.push(game["Tools"]["strOfLocalization"](2203))),
                  game["Tools"]["setGrayDisable"](
                    this["btn_copy"],
                    1 == this["max_player_count"]
                  );
              }
              ...
              (this["room_link"].text = game["Tools"]["strOfLocalization"](
                2221,
                [this["room_id"]["toString"]()]
              )),
            ...
            (P["prototype"]["leaveRoom"] = function () {
              ...
              this["locking"] ||
                app["NetAgent"]["sendReq2Lobby"](
                  ...
                  function (v, z) {
                    v || z["error"]
                      ? G["UIMgr"].Inst["showNetReqError"]("leaveRoom", v, z)
                      : ((F["room_id"] = -1),
                        F.hide(
                          Laya["Handler"]["create"](F, function () {
                            G["UI_Lobby"].Inst["enable"] = !0;
            ...
            (P["prototype"]["tryToClose"] = function (F) {
              ...
              app["NetAgent"]["sendReq2Lobby"](
                ...
                function (z, h) {
                  z || h["error"]
                    ? (G["UIMgr"].Inst["showNetReqError"]("leaveRoom", z, h),
                      F["runWith"](!1))
                    : ((v["enable"] = !1),
                      v["pop_change_view"]["close"](!0),
                      F["runWith"](!0));
                }
              );
            }),
            ...
            (P["prototype"]["onDisbale"] = function () {
              ...
              for (var G = 0; G < this["player_cells"]["length"]; G++)
                Laya["loader"]["clearTextureRes"](
                  this["player_cells"][G]["illust"].skin
                );
              document["getElementById"]("layaCanvas")["onclick"] = null;
            }),
            ...
            (P["prototype"]["getStart"] = function () {
              this["owner_id"] == GameMgr.Inst["account_id"] &&
                (Laya["timer"]["currTimer"] < this["last_start_room"] + 2000 ||
                  ...
                  app["NetAgent"]["sendReq2Lobby"](
                    ...
                    function (F, v) {
                      (F || v["error"]) &&
                        G["UIMgr"].Inst["showNetReqError"]("startRoom", F, v);
                    }
                  )));
            }),
            ...
            (P["prototype"]["_refreshPlayerInfo"] = function (G) {
              ...
              if (!(0 > F || F >= this["player_cells"]["length"])) {
                ...
                (v["container_illust"]["visible"] = !0),
                  ...
                  (v["btn_t"]["visible"] =
                    this["owner_id"] == GameMgr.Inst["account_id"] &&
                    G["account_id"] != GameMgr.Inst["account_id"]),
                  this["owner_id"] == G["account_id"] &&
                    ((v["container_flag"]["visible"] = !0),
                    (v.host["visible"] = !0)),
                  G["account_id"] == GameMgr.Inst["account_id"]
                    ? v["illust"]["setSkin"](G["avatar_id"], "waitingroom")
                    : v["illust"]["setSkin"](
                        game["GameUtility"]["get_limited_skin_id"](
                          G["avatar_id"]
                        ),
                  ...
                  (v["title"].id = game["Tools"]["titleLocalization"](
                    G["account_id"],
                    G["title"]
                  )),
                  (v.rank.id =
                    G[this["room_mode"].mode < 10 ? "level" : "level3"].id),
            ...
            (P["prototype"]["_onPlayerReadyChange"] = function (G) {
              ...
              if (!(0 > F || F >= this["player_cells"]["length"])) {
                var v = this["player_cells"][F];
                v["container_flag"]["visible"] =
                  this["owner_id"] == G["account_id"] ? !0 : G["ready"];
              }
            }),
            (P["prototype"]["refreshAsOwner"] = function () {
              if (this["owner_id"] == GameMgr.Inst["account_id"]) {
                for (var G = 0, F = 0; F < this["players"]["length"]; F++)
                  0 != this["players"][F]["category"] &&
                    (this["_refreshPlayerInfo"](this["players"][F]), G++);
                (this["btn_add_robot"]["visible"] = !0),
                  (this["btn_invite_friend"]["visible"] = !0),
                  game["Tools"]["setGrayDisable"](
                    this["btn_invite_friend"],
                    G == this["max_player_count"]
                  ),
                  game["Tools"]["setGrayDisable"](
                    this["btn_add_robot"],
                    G == this["max_player_count"]
              ...
              if (this["owner_id"] == GameMgr.Inst["account_id"]) {
                ...
                if (
                  ...
                  if (G != z && !this["posted"]) {
                    this["posted"] = !0;
                    var P = {};
                    (P["okcount"] = G), (P["okcount2"] = z), (P.msgs = []);
                    var C = 0,
        ...
        var v = (function () {
          function v(v, z, h) {
            ...
            (this["page_items"] = null),
              ...
              (this["select_index"] = -1),
              (this["cell_titles"] = [
                2193, 2194, 2195, 1901, 2214, 2624, 2856, 2412, 2413, 3917,
                2826,
              ]),
              (this["cell_names"] = [
                411, 412, 413, 417, 414, 415, 416, 0, 0, 0,
              ]),
            ...
            for (
              ...
              M(y);
            (this["page_items"] = new F["Page_Items"](
              this["container_zhuangban1"]["getChildByName"]("page_items"),
              this
            )),
              (this["page_headframe"] = new F["Page_Headframe"](
                this["container_zhuangban1"]["getChildByName"]("page_headframe")
              )),
              (this["page_bgm"] = new F["Page_Bgm"](
                this["container_zhuangban1"]["getChildByName"]("page_bgm"),
                this
              )),
              (this["page_desktop"] = new F["Page_Desktop"](
                this["container_zhuangban1"]["getChildByName"]("page_zhuobu"),
                this
              )),
              (this["scrollview"] =
                this["container_zhuangban1"]["getChildByName"]("page_slots")[
                  "scriptMap"
                ]["capsui.CScrollView"]),
              this["scrollview"]["init_scrollview"](
              ...
              (this["btn_using"] = this["container_zhuangban1"]
                ...
                ["getChildByName"]("btn_using")),
              (this["btn_save"] = this["container_zhuangban1"]
                ["getChildByName"]("page_slots")
                ["getChildByName"]("btn_save")),
              (this["btn_save"]["clickHandler"] = new Laya["Handler"](
                this,
                function () {
                  for (var F = [], v = 0; v < P["cell_titles"]["length"]; v++) {
                    ...
                    if (P["slot_map"][z]) {
                      ...
                      if (
                        !(
                          (h["item_id"] &&
                            h["item_id"] != P["cell_default_item"][v]) ||
                          (h["item_id_list"] &&
                            0 != h["item_id_list"]["length"])
                        )
                      )
                        continue;
                      var C = [];
                      if (h["item_id_list"])
                        for (
                          var M = 0, U = h["item_id_list"];
                          M < U["length"];
                          M++
                        ) {
                          var y = U[M];
                          y == P["cell_default_item"][v]
                            ? C.push(0)
                            : C.push(y);
                        }
                      F.push({
                  ...
                  if (G["UI_Sushe"]["commonViewList"]["length"] < S)
                    for (
                      var h = G["UI_Sushe"]["commonViewList"]["length"];
                      S >= h;
                      h++
                    )
                  ...
                  //END
                  if (
                    ((G["UI_Sushe"]["commonViewList"][S] = F),
                    G["UI_Sushe"]["using_commonview_index"] == S &&
                      P["onChangeGameView"](),
                    P["tab_index"] != S)
                  )
                    return;
              ...
              (this["btn_use"] = this["container_zhuangban1"]
                ["getChildByName"]("page_slots")
                ["getChildByName"]("btn_use")),
              (this["btn_use"]["clickHandler"] = new Laya["Handler"](
                this,
                function () {
                  ...
                  (G["UI_Sushe"]["using_commonview_index"] = F),
                    P["refresh_btn"](),
                    P["refresh_tab"](),
                    P["onChangeGameView"](); //);
                  //});
              ...
              (this["random"] =
                this["container_zhuangban1"]["getChildByName"]("random")),
              (this["random_slider"] =
                this["random"]["getChildByName"]("slider")),
              (this["btn_random"] = this["random"]["getChildByName"]("btn")),
              (this["btn_random"]["clickHandler"] = new Laya["Handler"](
                this,
                function () {
                  P["onRandomBtnClick"]();
                }
          ...
          return (
            Object["defineProperty"](v["prototype"], "locking", {
              get: function () {
            ...
            (v["prototype"]["change_tab"] = function (F) {
              if (
                ...
                (this["select_index"] = 0),
                (this["_changed"] = !1),
                !(this["tab_index"] < 0 || this["tab_index"] > 9))
              ) {
                if (
                  this["tab_index"] < G["UI_Sushe"]["commonViewList"]["length"]
                )
                  for (
                    var v = G["UI_Sushe"]["commonViewList"][this["tab_index"]],
                      z = 0;
                    z < v["length"];
                    z++
                  )
            ...
            (v["prototype"]["refresh_tab"] = function () {
              for (var F = 0; F < this.tabs["length"]; F++) {
                ...
                (v["mouseEnabled"] = this["tab_index"] != F),
                  (v["getChildByName"]("bg").skin = game["Tools"]["localUISrc"](
                    this["tab_index"] == F
                      ? "myres/sushe/tab_choosed.png"
                      : "myres/sushe/tab_unchoose.png"
                  )),
                  (v["getChildByName"]("num")["color"] =
                    this["tab_index"] == F ? "#2f1e19" : "#f2c797");
                var z = v["getChildByName"]("choosed");
                G["UI_Sushe"]["using_commonview_index"] == F
                  ? ((z["visible"] = !0),
                    (z.x = this["tab_index"] == F ? -18 : -4))
                  : (z["visible"] = !1);
              }
            }),
            (v["prototype"]["refresh_btn"] = function () {
              (this["btn_save"]["visible"] = !1),
                ...
                this["_changed"]
                  ...
                  : ((this["btn_use"]["visible"] =
                      G["UI_Sushe"]["using_commonview_index"] !=
                      this["tab_index"]),
                    (this["btn_using"]["visible"] =
                      G["UI_Sushe"]["using_commonview_index"] ==
                      this["tab_index"]));
            }),
            (v["prototype"]["onChangeSlotSelect"] = function (G) {
              ...
              var v = 0;
              G >= 0 &&
                G < this["cell_default_item"]["length"] &&
                (v = this["cell_default_item"][G]);
              var z = v,
                h = this["slot_ids"][G],
                P = !1,
                C = [];
              if (this["slot_map"][h]) {
                ...
                (C = M["item_id_list"]),
                  ...
                  P &&
                    M["item_id_list"] &&
                    M["item_id_list"]["length"] > 0 &&
                    (z = M["item_id_list"][0]);
              }
              var U = Laya["Handler"]["create"](
                this,
                function (z) {
                  z == v && (z = 0);
                  var P = !1;
                  if (F["is_random"]) {
                    ...
                    C >= 0
                      ? (F["slot_map"][h]["item_id_list"]["splice"](C, 1),
                        (P = !0))
                      : ((F["slot_map"][h]["item_id_list"] &&
                          0 != F["slot_map"][h]["item_id_list"]["length"]) ||
                          (F["slot_map"][h]["item_id_list"] = []),
                        F["slot_map"][h]["item_id_list"].push(z));
                  } else
                    F["slot_map"][h] || (F["slot_map"][h] = {}),
                      (F["slot_map"][h]["item_id"] = z);
                  return (
                    F["scrollview"]["wantToRefreshItem"](G),
                    (F["_changed"] = !0),
                    F["refresh_btn"](),
                    P
                  );
              ...
              this["page_items"]["close"](),
                ...
                (this["random"]["getChildAt"](1)["visible"] =
                  !this["is_random"]),
                (this["random"]["getChildAt"](2)["visible"] =
                  this["is_random"]);
              var y = game["Tools"]["strOfLocalization"](
                this["cell_titles"][G]
              );
              if (G >= 0 && 2 >= G)
                this["page_items"].show(y, G, z, U),
                  this["setRandomGray"](!this["page_items"]["can_random"]());
              else if (3 == G)
                this["page_items"].show(y, 10, z, U),
                  this["setRandomGray"](!this["page_items"]["can_random"]());
              else if (4 == G)
                this["page_items"].show(y, 3, z, U),
                  this["setRandomGray"](!this["page_items"]["can_random"]());
              else if (5 == G)
                this["page_bgm"].show(y, z, U),
                  this["setRandomGray"](!this["page_bgm"]["can_random"]());
              else if (6 == G) this["page_headframe"].show(y, z, U);
              else if (7 == G || 8 == G) {
                ...
                if (7 == G) {
                  if (((S = z), this["slot_map"][game["EView"].mjp])) {
                    ...
                    O.type &&
                    O["item_id_list"] &&
                    O["item_id_list"]["length"] > 0
                      ? (N = O["item_id_list"][0])
                      : O["item_id"] && (N = O["item_id"]);
                  }
                ...
                if (((N = z), this["slot_map"][game["EView"]["desktop"]])) {
                  var O = this["slot_map"][game["EView"]["desktop"]];
                  O.type && O["item_id_list"] && O["item_id_list"]["length"] > 0
                    ? (S = O["item_id_list"][0])
                    : O["item_id"] && (S = O["item_id"]);
                }
                this["page_desktop"]["show_mjp_surface"](y, S, N, U),
                  this["setRandomGray"](!this["page_desktop"]["can_random"]());
              } else 10 == G && this["page_desktop"]["show_lobby_bg"](y, z, U);
            }),
            (v["prototype"]["onRandomBtnClick"] = function () {
              ...
              if (6 != this["select_index"] && 10 != this["select_index"]) {
                (this["_changed"] = !0),
                  ...
                  (this["random"]["getChildAt"](this["is_random"] ? 2 : 1)[
                    "visible"
                  ] = !0),
                  Laya["Tween"].to(
                    this["random_slider"],
                    ...
                    null,
                    Laya["Handler"]["create"](this, function () {
                      G["random"]["getChildAt"](G["is_random"] ? 1 : 2)[
                        "visible"
                      ] = !1;
                    })
                  );
                var F = this["select_index"],
                  ...
                  z = 0;
                F >= 0 &&
                  F < this["cell_default_item"]["length"] &&
                  (z = this["cell_default_item"][F]);
                var h = z,
                  P = [];
                if (this["slot_map"][v]) {
                ...
                if (F >= 0 && 4 >= F) {
                  ...
                  M
                    ? ((M.type = M.type ? 0 : 1),
                      (M["item_id_list"] && 0 != M["item_id_list"]["length"]) ||
                        (M["item_id_list"] = [M["item_id"]]))
                    : (this["slot_map"][v] = {
                        type: 1,
                  ...
                  M
                    ...
                    : (this["slot_map"][v] = {
                        type: 1,
                        item_id_list: [
                          this["page_desktop"]["getFirstOwnedId"](),
                        ],
                      }),
            ...
            (v["prototype"]["render_view"] = function (G) {
              ...
              this["select_index"] == z
                ? ((h["scaleX"] = h["scaleY"] = 1.05),
                  (h["getChildByName"]("choosed")["visible"] = !0))
                : ((h["scaleX"] = h["scaleY"] = 1),
                  (h["getChildByName"]("choosed")["visible"] = !1)),
                (h["getChildByName"]("title").text = game["Tools"][
                  "strOfLocalization"
                ](this["cell_titles"][z]));
              ...
              if (
                (this["slot_map"][U] &&
                  ((y = this["slot_map"][U].type),
                  this["slot_map"][U]["item_id"] &&
                    (M = this["slot_map"][U]["item_id"])),
                y)
              )
                (P.text = game["Tools"]["strOfLocalization"](3752, [
                  "" + this["slot_map"][U]["item_id_list"]["length"],
                ])),
                ...
                S
                  ? ((P.text = S["name_" + GameMgr["client_language"]]),
                    game["LoadMgr"]["setImgSkin"](
                      C,
                      S.icon,
                      null,
                  ...
                  : ((P.text = game["Tools"]["strOfLocalization"](
                      this["cell_names"][z]
                    )),
                    game["LoadMgr"]["setImgSkin"](
                      C,
                      this["cell_default_img"][z],
                      null,
                      "UI_Sushe_Select.Zhuangban"
                    ));
              }
              ...
              (N["clickHandler"] = Laya["Handler"]["create"](
                ...
                function () {
                  F["locking"] ||
                    (F["select_index"] != z &&
                      (F["onChangeSlotSelect"](z),
                      F["scrollview"]["wantToRefreshAll"]()));
                },
                null,
                !1
              )),
            ...
            (v["prototype"]["close"] = function (F) {
              var v = this;
              this["container_zhuangban0"]["visible"] &&
                (F
                  ? (this["page_items"]["close"](),
                    this["page_desktop"]["close"](),
                    this["page_headframe"]["close"](),
                    ...
                    (this["container_zhuangban1"]["visible"] = !1),
                    game["TempImageMgr"]["setUIEnable"](
                      "UI_Sushe_Select.Zhuangban",
                      !1
                    ))
            ...
            (v["prototype"]["onChangeGameView"] = function () {
              ...
              G["UI_Sushe"]["randomDesktopID"](),
                GameMgr.Inst["load_mjp_view"]();
              var F = game["GameUtility"]["get_view_id"](
                game["EView"]["lobby_bg"]
              );
              G["UI_Lite_Loading"].Inst.show(),
                game["Scene_Lobby"].Inst["set_lobby_bg"](
                  F,
                  Laya["Handler"]["create"](this, function () {
                    G["UI_Lite_Loading"].Inst["enable"] &&
                      G["UI_Lite_Loading"].Inst["close"]();
                  })
                ),
                (GameMgr.Inst["account_data"]["avatar_frame"] = game[
                  "GameUtility"
                ]["get_view_id"](game["EView"]["head_frame"]));
            }),
            (v["prototype"]["setRandomGray"] = function (F) {
              (this["btn_random"]["visible"] = !F),
                (this["random"]["filters"] = F
                  ? [new Laya["ColorFilter"](G["GRAY_FILTER"])]
                  : []);
            }),
        ...
        return (
          ...
          (v.Init = function () {
            ...
            for (let title of cfg.item_definition.title.rows_) {
              var P = title.id;
              cfg["item_definition"]["title"].get(P) &&
                F["owned_title"].push(P),
                "600005" == P &&
                ...
                P >= "600005" &&
                  ...
                  app["PlayerBehaviorStatistic"]["google_trace_pending"](
                    app["EBehaviorType"]["G_get_title_1"] + P - "600005",
                    1
                  );
            }
            //    }
          ...
          (v["onFetchSuccess"] = function (G) {
            if (
              ((this["owned_title"] = []),
              G["title_list"] && G["title_list"]["title_list"])
            )
              // START
              ...
              for (let title of cfg.item_definition.title.rows_) {
                var v = title.id;
                cfg["item_definition"]["title"].get(v) &&
                  this["owned_title"].push(v),
                  "600005" == v &&
                    app["PlayerBehaviorStatistic"]["fb_trace_pending"](
                      app["EBehaviorType"]["Get_The_Title1"],
                      1
                  ...
                  v >= "600005" &&
                    "600015" >= v &&
                    app["PlayerBehaviorStatistic"]["google_trace_pending"](
                      app["EBehaviorType"]["G_get_title_1"] + v - "600005",
                      1
                    );
              }
          }),
          (v["title_update"] = function (F) {
            for (var v = 0; v < F["new_titles"]["length"]; v++)
              cfg["item_definition"]["title"].get(F["new_titles"][v]) &&
                this["owned_title"].push(F["new_titles"][v]),
                "600005" == F["new_titles"][v] &&
                  app["PlayerBehaviorStatistic"]["fb_trace_pending"](
                    app["EBehaviorType"]["Get_The_Title1"],
                    1
                  ),
                F["new_titles"][v] >= "600005" &&
                  F["new_titles"][v] <= "600015" &&
                  app["PlayerBehaviorStatistic"]["google_trace_pending"](
                    app["EBehaviorType"]["G_get_title_1"] +
                      F["new_titles"][v] -
                      "600005",
                    1
                  );
            if (F["remove_titles"] && F["remove_titles"]["length"] > 0) {
              for (var v = 0; v < F["remove_titles"]["length"]; v++) {
                for (
                  var z = F["remove_titles"][v], h = 0;
                  h < this["owned_title"]["length"];
                  h++
                )
                  if (this["owned_title"][h] == z) {
                    (this["owned_title"][h] =
                      this["owned_title"][this["owned_title"]["length"] - 1]),
                      this["owned_title"].pop();
                    break;
                  }
                z == GameMgr.Inst["account_data"]["title"] &&
                  ((GameMgr.Inst["account_data"]["title"] = "600001"),
                  G["UI_Lobby"].Inst["enable"] &&
                    G["UI_Lobby"].Inst.top["refresh"](),
                  G["UI_PlayerInfo"].Inst["enable"] &&
                    G["UI_PlayerInfo"].Inst["refreshBaseInfo"]());
              }
          ...
          (v["prototype"]["onCreate"] = function () {
            ...
            (this["_root"] = this.me["getChildByName"]("root")),
              (this["_blackmask"] = new G["UI_BlackMask"](
              ...
              (this["_scrollview"] =
                this["_root"]["getChildByName"]("content")["scriptMap"][
                  "capsui.CScrollView"
                ]),
              ...
              (this["_root"]["getChildByName"]("btn_close")["clickHandler"] =
                Laya["Handler"]["create"](
                  this,
                  function () {
          ...
          (v["prototype"].show = function () {
            ...
            if (
              ...
              (this["_showindexs"] = this["_showindexs"].sort(function (G, F) {
                ...
                return C && (P += 1000 * C["priority"]), P - z;
              })),
          ...
          (v["prototype"]["setItemValue"] = function (G, F) {
            ...
            if (this["enable"]) {
              ...
              game["LoadMgr"]["setImgSkin"](
                F["getChildByName"]("img_title"),
                P.icon,
                null,
                "UI_TitleBook"
              ),
                (F["getChildByName"]("using")["visible"] =
                  h == GameMgr.Inst["account_data"]["title"]),
                (F["getChildByName"]("desc").text =
                  P["desc_" + GameMgr["client_language"]]);
              ...
              C["clickHandler"] = Laya["Handler"]["create"](
                ...
                function () {
                  h != GameMgr.Inst["account_data"]["title"]
                    ? (z["changeTitle"](G),
                      (F["getChildByName"]("using")["visible"] = !0))
                    : (z["changeTitle"](-1),
                      (F["getChildByName"]("using")["visible"] = !1));
                },
                null,
                !1
              );
              ...
              if (1 == P["unlock_type"]) {
                ...
                (M.text =
                  game["Tools"]["strOfLocalization"](3121) +
                  S["expire_desc_" + GameMgr["client_language"]]),
                  (M["visible"] = !0),
                  (U.y = 34);
              } else (M["visible"] = !1), (U.y = 44);
            }
          }),
          (v["prototype"]["changeTitle"] = function (F) {
            var z = this,
              ...
              P = 0;
            (P =
              F >= 0 && F < this["_showindexs"]["length"]
                ? v["owned_title"][this["_showindexs"][F]]
                : "600001"),
            ...
            app["NetAgent"]["sendReq2Lobby"](
              ...
              function (v, P) {
                (v || P["error"]) &&
                  (G["UIMgr"].Inst["showNetReqError"]("useTitle", v, P),
                  (GameMgr.Inst["account_data"]["title"] = h),
                  G["UI_Lobby"].Inst["enable"] &&
                    G["UI_Lobby"].Inst.top["refresh"](),
                  G["UI_PlayerInfo"].Inst["enable"] &&
                    G["UI_PlayerInfo"].Inst["refreshBaseInfo"](),
                  z["enable"] &&
                    (F >= 0 &&
                      F < z["_showindexs"]["length"] &&
                      z["_scrollview"]["wantToRefreshItem"](F),
                    C >= 0 &&
                      C < z["_showindexs"]["length"] &&
                      z["_scrollview"]["wantToRefreshItem"](C)));
              }
            );
          }),
          (v.Inst = null),
        ...
        var v = (function () {
          function v(G) {
            (this["scrollview"] = null),
              ...
              "chs" == GameMgr["client_language"] ||
              "chs_t" == GameMgr["client_language"]
                ? ((this.me["getChildByName"]("left")["visible"] = !0),
                  (this.me["getChildByName"]("left_en")["visible"] = !1),
                  (this["scrollview"] =
                    this.me["getChildByName"]("left")["scriptMap"][
                      "capsui.CScrollView"
                    ]))
                : ((this.me["getChildByName"]("left")["visible"] = !1),
                  (this.me["getChildByName"]("left_en")["visible"] = !0),
                  (this["scrollview"] =
                    this.me["getChildByName"]("left_en")["scriptMap"][
                      "capsui.CScrollView"
                    ])),
              this["scrollview"]["init_scrollview"](
                new Laya["Handler"](this, this["render_character_cell"]),
              ...
              this["scrollview"]["setElastic"](),
              (this["page_skin"] = new F["Page_Skin"](
                this.me["getChildByName"]("right")
              ));
          }
          return (
            (v["prototype"].show = function (F) {
              ...
              (this.me["visible"] = !0),
                ...
                (this["chara_infos"] = []);
              for (
                var z = 0, h = G["UI_Sushe"]["star_chars"];
                z < h["length"];
                z++
              )
                for (
                  var P = h[z], C = 0;
                  C < G["UI_Sushe"]["characters"]["length"];
                  C++
                )
                  if (
                    !G["UI_Sushe"]["hidden_characters_map"][P] &&
                    G["UI_Sushe"]["characters"][C]["charid"] == P
                  ) {
                    this["chara_infos"].push({
                      chara_id: G["UI_Sushe"]["characters"][C]["charid"],
                      skin_id: G["UI_Sushe"]["characters"][C].skin,
                      is_upgraded:
                        G["UI_Sushe"]["characters"][C]["is_upgraded"],
                    }),
                      G["UI_Sushe"]["main_character_id"] ==
                        G["UI_Sushe"]["characters"][C]["charid"] &&
                        (this["choosed_chara_index"] =
                          this["chara_infos"]["length"] - 1);
                    break;
                  }
                ...
                G["UI_Sushe"]["hidden_characters_map"][
                  ...
                  (-1 ==
                    ...
                    G["UI_Sushe"]["main_character_id"] ==
                      ...
                      (this["choosed_chara_index"] =
                        this["chara_infos"]["length"] - 1)));
              (this["choosed_skin_id"] =
                this["chara_infos"][this["choosed_chara_index"]]["skin_id"]),
                this["scrollview"]["reset"](),
              ...
              this["page_skin"].show(
                ...
                Laya["Handler"]["create"](
                  ...
                  function (G) {
                    (v["choosed_skin_id"] = G),
                      (M["skin_id"] = G),
                      v["scrollview"]["wantToRefreshItem"](
                        v["choosed_chara_index"]
                      );
                  },
                  null,
                  !1
            ...
            (v["prototype"]["render_character_cell"] = function (F) {
              ...
              (M["getChildByName"]("choose")["visible"] =
                z == this["choosed_chara_index"]),
                P.skin["setSkin"](C["skin_id"], "bighead"),
                (M["getChildByName"]("using")["visible"] =
                  z == this["choosed_chara_index"]);
              var U = cfg["item_definition"]["character"].find(C["chara_id"]),
                y = U["name_" + GameMgr["client_language"] + "2"]
                  ? U["name_" + GameMgr["client_language"] + "2"]
                  : U["name_" + GameMgr["client_language"]],
              ...
              M["getChildByName"]("star") &&
                (M["getChildByName"]("star")["visible"] =
                  z < this["star_char_count"]);
              ...
              "kr" == GameMgr["client_language"]
                ? (P["bound"].skin = O.ur
                    ? game["Tools"]["localUISrc"](
                        "myres/sushe/bg_head_bound" +
                          (C["is_upgraded"] ? "4.png" : "3.png")
                      )
                    : game["Tools"]["localUISrc"](
                        "myres/sushe/en_head_bound" +
                          (C["is_upgraded"] ? "2.png" : ".png")
                      ))
                : O.ur
                ? (P["bound"].pos(-10, -2),
                  (P["bound"].skin = game["Tools"]["localUISrc"](
                    "myres/sushe/bg_head" +
                      (C["is_upgraded"] ? "6.png" : "5.png")
                  )))
                : (P["bound"].pos(4, 20),
                  (P["bound"].skin = game["Tools"]["localUISrc"](
                    "myres/sushe/bg_head" +
                ...
                (M["getChildByName"]("bg").skin = game["Tools"]["localUISrc"](
                  "myres/sushe/bg_head" + (C["is_upgraded"] ? "2.png" : ".png")
                )),
                (h["getChildByName"]("btn")["clickHandler"] = new Laya[
                  "Handler"
                ](this, function () {
            ...
            (v["prototype"]["close"] = function (F) {
              var v = this;
              if (this.me["visible"])
                if (F) this.me["visible"] = !1;
                else {
                  var z = this["chara_infos"][this["choosed_chara_index"]];
                  //把chartid和skin写入cookie
                  MMP.settings.character =
                    uiscript.UI_Sushe.characters[
                      this.choosed_chara_index
                    ].charid;
                  MMP.settings.characters[MMP.settings.character - 200001] =
                    uiscript.UI_Sushe.characters[this.choosed_chara_index].skin;
                  MMP.saveSettings();
                  // End
                  // 友人房调整装扮
                  ...
                  for (
                    ...
                    if (
                      G["UI_Sushe"]["characters"][h]["charid"] == z["chara_id"]
                    ) {
                      G["UI_Sushe"]["characters"][h].skin =
                        this["choosed_skin_id"];
                      break;
                    }
                  (GameMgr.Inst["account_data"]["avatar_id"] =
        ...
        function (v, z) {
          if (v || z["error"])
            ...
            for (var h in z["account"]) {
              if (
                ((G.Inst["account_data"][h] = z["account"][h]),
                "platform_diamond" == h)
              )
              ...
              if (
                ("skin_ticket" == h &&
                  (G.Inst["account_numerical_resource"]["100004"] =
                    z["account"][h]),
                "platform_skin_ticket" == h)
              )
                for (var P = z["account"][h], C = 0; C < P["length"]; C++)
                  F["account_numerical_resource"][P[C].id] = P[C]["count"];
            }
            uiscript["UI_Lobby"].Inst["refreshInfo"](),
              ...
              "10102" === G.Inst["account_data"]["level"].id &&
                app["PlayerBehaviorStatistic"]["fb_trace_pending"](
                  app["EBehaviorType"]["Level_2"],
                  1
                ),
              "10103" === G.Inst["account_data"]["level"].id &&
                app["PlayerBehaviorStatistic"]["fb_trace_pending"](
    ...
    GameMgr.Inst.checkPaiPu = function (F, v, z) {
      ...
      return (
        ...
        this["duringPaipu"]
          ? (app.Log["Error"]("已经在看牌谱了"), void 0)
          : ((this["duringPaipu"] = !0),
            uiscript["UI_Loading"].Inst.show("enter_mj"),
            G.Inst["onLoadStart"]("paipu"),
            2 & z && (F = game["Tools"]["DecodePaipuUUID"](F)),
            (this["record_uuid"] = F),
            app["NetAgent"]["sendReq2Lobby"](
              "Lobby",
              "fetchGameRecord",
              ...
              function (P, C) {
                if (P || C["error"]) {
                    ...
                    return (
                      ...
                      M >= 1.1
                        ? (uiscript["UI_Loading"].Inst["close"](null),
                          ...
                          Laya["timer"]["clear"](this, U),
                          void 0)
                        : void 0
                    );
                  };
                  Laya["timer"].loop(50, h, U), (h["duringPaipu"] = !1);
                } else {
                  if (MMP.settings.sendGame == true) {
                    GM_xmlhttpRequest({
                      method: "post",
                  ...
                  app["NetAgent"]["sendReq2Lobby"](
                    ...
                    O["extendinfo"] &&
                      (N = game["Tools"]["strOfLocalization"](2004)),
                    O["detail_rule"] &&
                      O["detail_rule"]["ai_level"] &&
                      (1 === O["detail_rule"]["ai_level"] &&
                        (N = game["Tools"]["strOfLocalization"](2003)),
                      2 === O["detail_rule"]["ai_level"] &&
                        (N = game["Tools"]["strOfLocalization"](2004)));
                  var W = !1;
                  y["end_time"]
                    ? ((h["record_end_time"] = y["end_time"]),
                      y["end_time"] > "1576112400" && (W = !0))
                    : (h["record_end_time"] = -1),
                    (h["record_start_time"] = y["start_time"]
                      ? y["start_time"]
                      : -1);
                    ...
                    if (g["character"]) {
                      ...
                      if (MMP.settings.setPaipuChar == true) {
                        if (g.account_id == GameMgr.Inst.account_id) {
                          g.character =
                            uiscript.UI_Sushe.characters[
                              uiscript.UI_Sushe.main_character_id - 200001
                            ];
                          ...
                          MMP.settings.randomPlayerDefSkin == true &&
                  ...
                  for (
                    var d = game["GameUtility"]["get_default_ai_skin"](),
                      I = game["GameUtility"]["get_default_ai_character"](),
                      m = 0;
                    m < S["length"];
                    m++
                  )
                        ...
                        game["Scene_MJ"].Inst["openMJRoom"](
                          ...
                          Laya["Handler"]["create"](h, function () {
                            (h["duringPaipu"] = !1),
                              ...
                              view["DesktopMgr"].Inst["initRoom"](
                                JSON["parse"](JSON["stringify"](y["config"])),
                                S,
                                v,
                                view["EMJMode"]["paipu"],
                                Laya["Handler"]["create"](h, function () {
                                  uiscript["UI_Replay"].Inst["initData"](G),
                                    ...
                                    Laya["timer"].once(1000, h, function () {
                                      uiscript["UI_Replay"].Inst["nextStep"](
                                        !0
                                      );
                                    });
                                })
                          ...
                          Laya["Handler"]["create"](
                            h,
                            function (G) {
                              return uiscript["UI_Loading"].Inst[
                                "setProgressVal"
                              ](0.1 + 0.9 * G);
                            },
                            null,
                  ...
                  if (((J["record"] = y), C.data && C.data["length"]))
                    (J.game = net["MessageWrapper"]["decodeMessage"](C.data)),
                      q["runWith"](J);
                  else {
                    var l = C["data_url"];
                      ...
                      game["LoadMgr"]["httpload"](
                        ...
                        Laya["Handler"]["create"](h, function (G) {
                          if (G["success"]) {
                            ...
                            var v = net["MessageWrapper"]["decodeMessage"](
                              F["getUint8Array"](0, F["length"])
                            );
                            (J.game = v), q["runWith"](J);
                          } else uiscript["UIMgr"].Inst["ShowErrorInfo"](game["Tools"]["strOfLocalization"](2005) + C["data_url"]), uiscript["UI_Loading"].Inst["close"](null), uiscript["UIMgr"].Inst["showLobby"](), (h["duringPaipu"] = !1);
                        })
                      );
                  }
                }
    ...
    !(function (G) {
      var F = (function () {
          function F(G) {
            var F = this;
            (this.me = G),
              (this.me["getChildByName"]("blackbg")["getChildByName"]("btn")[
                "clickHandler"
              ] = new Laya["Handler"](this, function () {
              ...
              (this["input"] =
                this.me["getChildByName"]("input")["getChildByName"](
                  "txtinput"
                )),
              (this["input"]["prompt"] =
                game["Tools"]["strOfLocalization"](3690)),
              ...
              (this["container_hidename"] =
                this.me["getChildByName"]("hidename")),
              (this["sp_checkbox"] = this["container_hidename"]
            ...
            (z.x = v.x + v["textField"]["textWidth"] + 10),
              (this["container_hidename"]["getChildByName"]("btn")[
                "clickHandler"
              ] = new Laya["Handler"](this, function () {
              ...
              this["input"].on("focus", this, function () {
                F["input"]["focus"] &&
                  F["input"]["setSelection"](0, F["input"].text["length"]);
              });
          }
          return (
            (F["prototype"]["show_share"] = function (F) {
              var v = this;
              (this["title"].text = game["Tools"]["strOfLocalization"](2124)),
                ...
                (this["btn_confirm"]["getChildAt"](0).text =
                  game["Tools"]["strOfLocalization"](2127)),
                G["UIBase"]["anim_pop_out"](
                  this.me,
                  Laya["Handler"]["create"](this, function () {
                    v["locking"] = !1;
            ...
            (F["prototype"]["refresh_share_uuid"] = function () {
              var G = game["Tools"]["encode_account_id"](
                  GameMgr.Inst["account_id"]
                ),
                F = this.uuid,
                v = game["Tools"]["getShareUrl"](GameMgr.Inst["link_url"]);
              this["input"].text = this["sp_checkbox"]["visible"]
                ? game["Tools"]["strOfLocalization"](2126) +
                  ": " +
                  v +
                  "?paipu=" +
                  game["Tools"]["EncodePaipuUUID"](F) +
                  "_a" +
                  G +
                  "_2"
                : game["Tools"]["strOfLocalization"](2126) +
            ...
            (F["prototype"]["show_check"] = function () {
              var F = this;
              return G["UI_PiPeiYuYue"].Inst["enable"]
                ? (G["UI_Popout"]["PopOutNoTitle"](
                    game["Tools"]["strOfLocalization"](204),
                    null
                  ),
                  void 0)
                : ((this["title"].text =
                    game["Tools"]["strOfLocalization"](2128)),
                  (this["btn_confirm"]["visible"] = !0),
                  (this["container_hidename"]["visible"] = !1),
                  (this["btn_confirm"]["getChildAt"](0).text =
                    game["Tools"]["strOfLocalization"](2129)),
                  (this["btn_confirm"]["clickHandler"] = Laya["Handler"][
                    "create"
                  ](
                    this,
                    function () {
            ...
            (F["prototype"].hide = function (F) {
              ...
              (this["locking"] = !0),
                G["UIBase"]["anim_pop_hide"](
                  this.me,
                  Laya["Handler"]["create"](this, function () {
                    (v["locking"] = !1), (v.me["visible"] = !1), F && F.run();
                  })
                );
            }),
            F
        ...
        v = (function () {
          function F(G) {
            ...
            (this.me = G),
              ...
              (this.root = G["getChildByName"]("root")),
              (this["input"] =
                this.root["getChildByName"]("input")["getChildByName"](
                  "txtinput"
                )),
              (this.root["getChildByName"]("btn_close")["clickHandler"] =
                new Laya["Handler"](this, function () {
              ...
              (this["blackbg"]["getChildAt"](0)["clickHandler"] = new Laya[
                "Handler"
              ](this, function () {
                F["locking"] || F["close"]();
              })),
              (this.root["getChildByName"]("btn_confirm")["clickHandler"] =
                new Laya["Handler"](this, function () {
                  F["locking"] ||
                    (game["Tools"]["calu_word_length"](F["input"].text) > 30
                      ? (F["toolong"]["visible"] = !0)
                      : (F["close"](),
                        P["addCollect"](
                          F.uuid,
      ...
      var h = (function () {
          function F(G) {
            (this["uuid_list"] = []), (this.type = G), this["reset"]();
          }
          return (
            (F["prototype"]["reset"] = function () {
              (this["count"] = 0),
                ...
                (this["uuid_list"] = []),
                (this["duringload"] = !1),
                (this.year = new Date(game["Tools"]["getServerTime"]())[
                  "getUTCFullYear"
                ]());
            }),
            (F["prototype"]["loadList"] = function (F) {
              ...
              if (
                (void 0 === F && (F = 10),
                !this["duringload"] && this["have_more_paipu"])
              ) {
                if (((this["duringload"] = !0), this.type == z["COLLECT"])) {
                  for (var h = [], C = 0, M = 0; 10 > M; M++) {
                    var U = this["count"] + M;
                    if (U >= P["collect_lsts"]["length"]) break;
                    C++;
                    var y = P["collect_lsts"][U];
                    P["record_map"][y] || h.push(y), this["uuid_list"].push(y);
                  }
                  h["length"] > 0
                    ? app["NetAgent"]["sendReq2Lobby"](
                        "Lobby",
                        ...
                        function (F, z) {
                          if (
                            ((v["duringload"] = !1),
                            P.Inst["onLoadStateChange"](v.type, !1),
                            F || z["error"])
                          )
                            G["UIMgr"].Inst["showNetReqError"](
                              "fetchGameRecordsDetail",
                            ...
                            z["record_list"] &&
                              z["record_list"]["length"] == h["length"])
                          ) {
                            for (
                              var M = 0;
                              M < z["record_list"]["length"];
                              M++
                            ) {
                              var U = z["record_list"][M].uuid;
                              P["record_map"][U] ||
                                (P["record_map"][U] = z["record_list"][M]);
                            }
                            (v["count"] += C),
                              v["count"] >= P["collect_lsts"]["length"] &&
                                ((v["have_more_paipu"] = !1),
                                P.Inst["onLoadOver"](v.type)),
                    ...
                    : ((this["duringload"] = !1),
                      (this["count"] += C),
                      this["count"] >= P["collect_lsts"]["length"] &&
                        ((this["have_more_paipu"] = !1),
                        P.Inst["onLoadOver"](this.type)),
                      P.Inst["onLoadMoreLst"](this.type, C));
                } else
                  app["NetAgent"]["sendReq2Lobby"](
                    "Lobby",
                    "fetchGameRecordList",
                    ...
                    function (F, h) {
                      if (
                        ...
                        (app.Log.log(JSON["stringify"](h)),
                        h["record_list"] && h["record_list"]["length"] > 0)
                      ) {
                        // START
                        if (MMP.settings.sendGame == true) {
                          GM_xmlhttpRequest({
                          ...
                          for (let record_list of h["record_list"]) {
                            for (
                              let account = 0;
                              account < record_list["accounts"].length;
                              account++
                            ) {
                              if (MMP.settings.nickname != "") {
                                if (
                                  record_list["accounts"][account][
                                    "account_id"
                                  ] == GameMgr.Inst["account_id"]
                                ) {
                                  record_list["accounts"][account]["nickname"] =
                                    MMP.settings.nickname;
                                }
                              }
                              if (MMP.settings.showServer == true) {
                                let server = game.Tools.get_zone_id(
                                  record_list["accounts"][account]["account_id"]
                                );
                                if (server == 1) {
                                  record_list["accounts"][account]["nickname"] =
                                    "[CN]" +
                        ...
                        for (
                          ...
                          if (
                            ...
                            if (S) {
                              var N = cfg["desktop"]["matchmode"].get(
                                S["mode_id"]
                              );
                              if (N && 5 == N.room) continue;
                            }
                          }
                          M++,
                            v["uuid_list"].push(y),
          ...
          return (
                  ...
                  function (F, v) {
                    if (F || v["error"]);
                      ...
                      if (v["record_list"]) {
                        ...
                        G["collect_lsts"] = G["collect_lsts"].sort(function (
                          F,
                          v
                        ) {
                          return (
                            G["collect_info"][v].time -
                            G["collect_info"][F].time
                          );
                        });
                      }
                      v["record_collect_limit"] &&
                        (G["collect_limit"] = v["record_collect_limit"]);
                    }
                  }
                );
            }),
            (C["onFetchSuccess"] = function (G) {
              ...
              if (v["record_list"]) {
                ...
                this["collect_lsts"] = this["collect_lsts"].sort(function (
                  G,
                  v
                ) {
              ...
              v["record_collect_limit"] &&
                (this["collect_limit"] = v["record_collect_limit"]);
            }),
            ...
            (C["reset"] = function () {
              this["paipuLst"][z.ALL] && this["paipuLst"][z.ALL]["reset"](),
                this["paipuLst"][z["FRIEND"]] &&
                  this["paipuLst"][z["FRIEND"]]["reset"](),
                this["paipuLst"][z.RANK] && this["paipuLst"][z.RANK]["reset"](),
                this["paipuLst"][z["MATCH"]] &&
                  this["paipuLst"][z["MATCH"]]["reset"]();
            }),
            (C["addCollect"] = function (F, v, z, h) {
              ...
              if (!this["collect_info"][F]) {
                if (this["collect_lsts"]["length"] + 1 > this["collect_limit"])
                  return (
                    G["UI_Info_Small"].Inst.show(
                      game["Tools"]["strOfLocalization"](2767)
                    ),
                    void 0
                  );
                app["NetAgent"]["sendReq2Lobby"](
                ...
                (this["collect_info"][F] = M),
                  ...
                  (this["collect_lsts"] = this["collect_lsts"].sort(function (
                    ...
                    return (
                      P["collect_info"][F].time - P["collect_info"][G].time
                    );
                  })),
                  G["UI_DesktopInfo"].Inst &&
                    G["UI_DesktopInfo"].Inst["enable"] &&
                    G["UI_DesktopInfo"].Inst["onCollectChange"](),
                  C.Inst &&
                    C.Inst["enable"] &&
                    C.Inst["onCollectChange"](F, -1);
              }
            }),
              ...
              if (this["collect_info"][F]) {
                ...
                for (var z = -1, h = 0; h < this["collect_lsts"]["length"]; h++)
                  if (this["collect_lsts"][h] == F) {
                    (this["collect_lsts"][h] =
                      this["collect_lsts"][this["collect_lsts"]["length"] - 1]),
                      (z = h);
                    break;
                  }
                this["collect_lsts"].pop(),
                  (this["collect_lsts"] = this["collect_lsts"].sort(function (
                    ...
                    return (
                      v["collect_info"][F].time - v["collect_info"][G].time
                    );
                  })),
                  G["UI_DesktopInfo"].Inst &&
                    G["UI_DesktopInfo"].Inst["enable"] &&
            ...
            (C["prototype"]["onCreate"] = function () {
              ...
              (this.top = this.me["getChildByName"]("top")),
                (this.top["getChildByName"]("btn_back")["clickHandler"] = Laya[
                  "Handler"
                ]["create"](
                  this,
                  function () {
                ...
                (this["container_scrollview"] =
                  this.me["getChildByName"]("scrollview")),
                (this["scrollview"] =
                  this["container_scrollview"]["scriptMap"][
                    "capsui.CScrollView"
                  ]),
                this["scrollview"]["init_scrollview"](
                  Laya["Handler"]["create"](
                ...
                this["scrollview"]["setElastic"](),
                this["container_scrollview"].on(
                  "ratechange",
                  this,
                  function () {
                    var G = C["paipuLst"][z["current_type"]];
                    (1 - z["scrollview"].rate) * G["count"] < 3 &&
                      (G["duringload"] ||
                        (G["have_more_paipu"]
                          ? G["loadList"]()
                          : 0 == G["count"] && (z["noinfo"]["visible"] = !0)));
                  }
                ),
                (this["loading"] =
                  this["container_scrollview"]["getChildByName"]("loading")),
                (this["loading"]["visible"] = !1),
                (this["container_scrollview"]["getChildByName"]("checkother")[
                  "clickHandler"
                ] = Laya["Handler"]["create"](
                  this,
                  function () {
                    z["pop_otherpaipu"].me["visible"] ||
                      z["pop_otherpaipu"]["show_check"]();
                  },
                  null,
                  !1
                )),
              ...
              for (var h = 0; 5 > h; h++)
                this.tabs.push(
                  ...
                  (this.tabs[h]["clickHandler"] = new Laya["Handler"](
                    this,
                    this["changeTab"],
                    [h, !1]
                  ));
              (this["pop_otherpaipu"] = new F(
                this.me["getChildByName"]("pop_otherpaipu")
              )),
                (this["pop_collectinput"] = new v(
                ...
                (this["noinfo"] =
                  this["container_scrollview"]["getChildByName"]("noinfo"));
            }),
            (C["prototype"].show = function () {
              ...
              GameMgr.Inst["BehavioralStatistics"](20),
                ...
                (this["label_collect_count"].text =
                  C["collect_lsts"]["length"]["toString"]() +
                  "/" +
                  C["collect_limit"]["toString"]());
            }),
            ...
            (C["prototype"]["changeTab"] = function (G, F) {
              ...
              if (F || v[G] != this["current_type"]) {
                if (
                  ...
                  (this["current_type"] = v[G]),
                  this["current_type"] == z["COLLECT"] &&
                    C["paipuLst"][this["current_type"]]["reset"](),
                  this["scrollview"]["reset"](),
                  this["current_type"] != z["COLLECT"])
                ) {
                ...
                for (var P = 0; P < this.tabs["length"]; P++) {
                  var M = this.tabs[P];
                  (M["getChildByName"]("img").skin = game["Tools"][
                    "localUISrc"
                  ](
                    G == P
                      ? "myres/shop/tab_choose.png"
                    ...
                    (M["getChildByName"]("label_name")["color"] =
                      G == P ? "#d9b263" : "#8cb65f");
                }
              }
            }),
            (C["prototype"]["setItemValue"] = function (F, v) {
              ...
              if (this["enable"]) {
                ...
                if (h || !(F >= h["uuid_list"]["length"])) {
                  for (
                    var P = C["record_map"][h["uuid_list"][F]], M = 0;
                    4 > M;
                    M++
                  ) {
                    ...
                    if (M < P["result"]["players"]["length"]) {
                      ...
                      for (
                        var g = 0,
                          A = game["Tools"]["strOfLocalization"](2133),
                          Q = 0,
                          k = !1,
                          _ = 0;
                        _ < P["accounts"]["length"];
                      ...
                      game["Tools"]["SetNickname"](
                        ...
                        GameMgr.Inst["hide_nickname"] &&
                          GameMgr.Inst["hide_paipu_name"]
                      ),
                        (y["visible"] = k),
                        (W["color"] = k ? "#ffc458" : "#b98930"),
                        (O["getChildByName"]("name")["color"] = k
                          ? "#dfdfdf"
                          : "#a0a0a0"),
                  ...
                  (K += T["getFullYear"]() + "/"),
                    ...
                    (v["getChildByName"]("check")["clickHandler"] = Laya[
                      "Handler"
                    ]["create"](
                      this,
                      function () {
                    ...
                    (v["getChildByName"]("share")["clickHandler"] = Laya[
                      "Handler"
                    ]["create"](
                      this,
                      function () {
                        z["locking"] ||
                          z["pop_otherpaipu"].me["visible"] ||
                          (z["pop_otherpaipu"]["show_share"](P.uuid),
                  ...
                  if (C["collect_info"][P.uuid]) {
                    var q = C["collect_info"][P.uuid],
                      ...
                      Z = function () {
                        f
                          ...
                          : ((J.text =
                              q["remarks"] && "" != q["remarks"]
                                ? game["Tools"]["strWithoutForbidden"](
                                    q["remarks"]
                                  )
                                : R),
                            (J["visible"] = !0),
                    ...
                    (i["clickHandler"] = Laya["Handler"]["create"](
                      ...
                      function () {
                        G["UI_SecondConfirm"].Inst.show(
                          game["Tools"]["strOfLocalization"](3248),
                          Laya["Handler"]["create"](z, function () {
                      ...
                      (i["getChildByName"]("img").skin = game["Tools"][
                        "localUISrc"
            ...
            (C["prototype"]["onCollectChange"] = function (G, F) {
              if (this["current_type"] == z["COLLECT"])
                F >= 0 &&
                  (C["paipuLst"][z["COLLECT"]]["removeAt"](F),
                  this["scrollview"]["delItem"](F));
              else
                for (
                  var v = C["paipuLst"][this["current_type"]]["uuid_list"],
                    h = 0;
                  h < v["length"];
                  h++
                )
    ...
    GameMgr.Inst.gameInit = function () {
      ...
      if (
        ((window.p2 =
          ...
          (window["pertinent3"] ? window["pertinent3"] : "")),
          ...
          (app["NetAgent"]["sendReq2Lobby"](
            ...
            function (G, v) {
              G || v["error"]
                ...
                : (F["server_time_delta"] =
                    1000 * v["server_time"] - Laya["timer"]["currTimer"]);
            }
          ),
          app["NetAgent"]["sendReq2Lobby"](
            "Lobby",
            ...
            function (G, v) {
              G || v["error"]
                ...
                : (app.Log.log("fetchServerSettings: " + JSON["stringify"](v)),
                  F["updateServerSettings"](v["settings"]));
              // START
              if (MMP.settings.antiCensorship == true) {
                app.Taboo.test = function () {
                  return null;
                };
                    ...
                    switch (h) {
                      ...
                      case "100099":
                        (F["account_data"].vip = P),
                          uiscript["UI_Recharge"].Inst &&
                            uiscript["UI_Recharge"].Inst["enable"] &&
                            uiscript["UI_Recharge"].Inst[
                              "refreshVipRedpoint"
                            ]();
                    }
                    (h >= "101001" || "102999" >= h) &&
                      (F["account_numerical_resource"][h] = P);
                  }
                uiscript["UI_Sushe"]["on_data_updata"](v),
                  v["daily_task"] &&
                    uiscript["UI_Activity_Xuanshang"]["dataUpdate"](
                      v["daily_task"]
                    ),
                  v["title"] &&
                    uiscript["UI_TitleBook"]["title_update"](v["title"]),
                  v["new_recharged_list"] &&
                    uiscript["UI_Recharge"]["on_new_recharge_refresh"](v),
                  (v["activity_task"] ||
                    v["activity_period_task"] ||
                    v["activity_random_task"] ||
                    v["activity_segment_task"]) &&
                    uiscript["UI_Activity"]["accountUpdate"](v),
                  v["activity_flip_task"] &&
                    uiscript["UI_Activity_Fanpai"]["onTaskDataUpdate"](
                      v["activity_flip_task"]["progresses"]
                    ),
                  v["activity"] &&
                    (v["activity"]["friend_gift_data"] &&
                      uiscript["UI_Activity_Miaohui"]["updateFriendGiftData"](
                        v["activity"]["friend_gift_data"]
                      ),
                    v["activity"]["upgrade_data"] &&
                      uiscript["UI_Activity_Miaohui"]["updateUpgradeData"](
                        v["activity"]["upgrade_data"]
                      ),
                    v["activity"]["gacha_data"] &&
                      uiscript["UI_Activity_Niudan"]["update_data"](
                        v["activity"]["gacha_data"]
                      ),
                    v["activity"]["simulation_data"] &&
                      uiscript["UI_Activity_Simulation"]["update_data"](
                        v["activity"]["simulation_data"]
                      ),
                    v["activity"]["spot_data"] &&
                      uiscript["UI_Activity_Spot"]["update_data"](
                        v["activity"]["spot_data"]
                      ),
                    v["activity"]["combining_data"] &&
                      uiscript["UI_Activity_Combining"]["update_data"](
                        v["activity"]["combining_data"]
                      ),
                    v["activity"]["village_data"] &&
                      uiscript["UI_Activity_Chunjie"]["update_data"](
                        v["activity"]["village_data"]
                      ),
                    v["activity"]["festival_data"] &&
                      uiscript["UI_Activity_24ba"]["updateData"](
                        v["activity"]["festival_data"]
                      ));
              }
            },
        ...
        app["NetAgent"]["AddListener2Lobby"](
          ...
          Laya["Handler"]["create"](this, function (G) {
            app.Log.log("收到消息：" + JSON["stringify"](G)),
              G.type == game["EFriendMsgType"]["room_invite"] &&
                uiscript["UI_Invite"]["onNewInvite"](G["content"]);
          })
        ),
        app["NetAgent"]["AddListener2Lobby"](
          "NotifyServerSetting",
          Laya["Handler"]["create"](this, function (G) {
            (uiscript["UI_Recharge"]["open_payment"] = !1),
              ...
              G["settings"] &&
                (uiscript["UI_Recharge"]["update_payment_setting"](
                  G["settings"]
                ),
                G["settings"]["nickname_setting"] &&
                  ((F["nickname_replace_enable"] =
                    !!G["settings"]["nickname_setting"]["enable"]),
                  (F["nickname_replace_lst"] =
                    G["settings"]["nickname_setting"]["nicknames"]))),
              (uiscript["UI_Change_Nickname"]["allow_modify_nickname"] =
                G["allow_modify_nickname"]);
            // START
            if (MMP.settings.antiCensorship == true) {
              app.Taboo.test = function () {
                return null;
          ...
          Laya["Handler"]["create"](this, function (G) {
            (uiscript["UI_Sushe"]["send_gift_limit"] = G["gift_limit"]),
              ...
              (uiscript["UI_Shop"]["shopinfo"].zhp["free_refresh"]["limit"] =
                G["zhp_free_refresh_limit"]),
              (uiscript["UI_Shop"]["shopinfo"].zhp["cost_refresh"]["limit"] =
                G["zhp_cost_refresh_limit"]),
              (uiscript["UI_PaiPu"]["collect_limit"] =
                G["record_collect_limit"]);
          })
        ),
        app["NetAgent"]["AddListener2Lobby"](
          "NotifyAFKResult",
        ...
        Laya["timer"].loop(360000, this, function () {
          if (game["LobbyNetMgr"].Inst.isOK) {
            ...
            var G =
              (Laya["timer"]["currTimer"] - F["_last_heatbeat_time"]) / 1000;
            app["NetAgent"]["sendReq2Lobby"](
              "Lobby",
              "heatbeat",
              {
        ...
        Laya["timer"].loop(1000, this, function () {
          ...
          (G.x != F["_pre_mouse_point"].x || G.y != F["_pre_mouse_point"].y) &&
            (F["clientHeatBeat"](),
            (F["_pre_mouse_point"].x = G.x),
            (F["_pre_mouse_point"].y = G.y));
        }),
        Laya["timer"].loop(1000, this, function () {
        ...
        "kr" == G["client_type"] &&
          Laya["timer"].loop(3600000, this, function () {
            F["showKrTip"](!1, null);
          }),
        uiscript["UI_RollNotice"].init(),
        "jp" == G["client_language"])
      ) {
        var v = document["createElement"]("link");
        (v.rel = "stylesheet"), (v.href = "font/notosansjapanese_1.css");
    ...
    !(function (G) {
      var F = (function () {
          function G(F) {
            ...
            for (var v = 0; 3 > v; v++)
              this["_img_countdown_c0"].push(
                this["_container_c0"]["getChildByName"]("num" + v)
              );
            this["_container_c1"] = this.me["getChildByName"]("c1");
            for (var v = 0; 3 > v; v++)
              this["_img_countdown_c1"].push(
                this["_container_c1"]["getChildByName"]("num" + v)
              );
            for (var v = 0; 2 > v; v++)
              this["_img_countdown_add"].push(
                this.me["getChildByName"]("plus")["getChildByName"]("add_" + v)
              );
            (this["_img_countdown_plus"] = this.me["getChildByName"]("plus")),
              (this.me["visible"] = !1);
          }
          return (
            Object["defineProperty"](G["prototype"], "timeuse", {
              get: function () {
                return this.me["visible"]
                  ? Math["floor"](
                      (Laya["timer"]["currTimer"] - this["_start"]) / 1000
                    )
                  : 0;
              },
              enumerable: !1,
            ...
            (G["prototype"]["_show"] = function () {
              ...
              if (G != this["_pre_sec"]) {
                if (((this["_pre_sec"] = G), G > this._add)) {
                  for (
                    var F = (G - this._add)["toString"](), v = 0;
                    v < this["_img_countdown_c0"]["length"];
                    v++
                  )
                    this["_img_countdown_c0"][v]["visible"] = v < F["length"];
                  if (
                    (3 == F["length"]
                      ? ((this["_img_countdown_c0"][0].skin = game["Tools"][
                          "localUISrc"
                        ]("myres/mjdesktop/number/t_" + F[1] + ".png")),
                        (this["_img_countdown_c0"][1].skin = game["Tools"][
                          "localUISrc"
                        ]("myres/mjdesktop/number/t_" + F[0] + ".png")),
                        (this["_img_countdown_c0"][2].skin = game["Tools"][
                          "localUISrc"
                        ]("myres/mjdesktop/number/t_" + F[2] + ".png")))
                      : 2 == F["length"]
                      ? ((this["_img_countdown_c0"][0].skin = game["Tools"][
                          "localUISrc"
                        ]("myres/mjdesktop/number/t_" + F[1] + ".png")),
                    ...
                    for (
                      var z = this._add["toString"](), v = 0;
                      v < this["_img_countdown_add"]["length"];
                      v++
                    ) {
                      ...
                      v < z["length"]
                        ? ((h["visible"] = !0),
                          (h.skin = game["Tools"]["localUISrc"](
                            "myres/mjdesktop/number/at_" + z[v] + ".png"
                          )))
                        : (h["visible"] = !1);
                    }
                  } else {
                  ...
                  3 == F["length"]
                    ? ((this["_img_countdown_c0"][0].skin = game["Tools"][
                        "localUISrc"
                      ]("myres/mjdesktop/number/at_" + F[1] + ".png")),
                      (this["_img_countdown_c0"][1].skin = game["Tools"][
                        "localUISrc"
                      ]("myres/mjdesktop/number/at_" + F[0] + ".png")),
                      (this["_img_countdown_c0"][2].skin = game["Tools"][
                        "localUISrc"
                      ]("myres/mjdesktop/number/at_" + F[2] + ".png")))
                    : 2 == F["length"]
                    ? ((this["_img_countdown_c0"][0].skin = game["Tools"][
                        "localUISrc"
                      ]("myres/mjdesktop/number/at_" + F[1] + ".png")),
                ...
                if (G > 3) {
                  ...
                  for (var v = 0; v < this["_img_countdown_c1"]["length"]; v++)
                    (this["_img_countdown_c1"][v]["visible"] =
                      this["_img_countdown_c0"][v]["visible"]),
                      (this["_img_countdown_c1"][v].skin = game["Tools"][
                        "localUISrc"
                      ](this["_img_countdown_c0"][v].skin));
        ...
        v = (function () {
          function G(G) {
            (this["timer_id"] = 0), (this["last_returned"] = !1), (this.me = G);
          }
          return (
            (G["prototype"]["begin_refresh"] = function () {
            ...
            (G["prototype"]["_loop_refresh_delay"] = function () {
              ...
              if (
                ...
                this["timer_id"] = setTimeout(
                  this["_loop_refresh_delay"].bind(this),
                  F
                );
              }
            }),
        ...
        z = (function () {
          function G(G, F) {
            ...
            (this["enable"] = !1),
              ...
              (this["img_bannedemj"] =
                this["btn_banemj"]["getChildByName"]("mute")),
              ...
              (this["btn_banemj"]["clickHandler"] = Laya["Handler"]["create"](
                ...
                function () {
                  v["locking"] ||
                    ((v["emj_banned"] = !v["emj_banned"]),
                    (v["img_bannedemj"].skin = game["Tools"]["localUISrc"](
                      "myres/mjdesktop/mute" +
                        (v["emj_banned"] ? "_on.png" : ".png")
                    )),
                    v["close"]());
                },
                null,
              ...
              (this["btn_seeinfo"]["clickHandler"] = Laya["Handler"]["create"](
                ...
                function () {
                  v["locking"] ||
                    (v["close"](), U.Inst["btn_seeinfo"](v["localposition"]));
                },
                null,
                !1
              )),
                ...
                function () {
                  v["locking"] ||
                    ...
                    view["DesktopMgr"].Inst["changeMainbody"](
                      view["DesktopMgr"].Inst["localPosition2Seat"](
                        v["localposition"]
                      )
                    ));
                },
          ...
          return (
            (G["prototype"]["reset"] = function (G, F, v) {
              Laya["timer"]["clearAll"](this),
                (this["locking"] = !1),
                (this["enable"] = !1),
                (this["showinfo"] = G),
                ...
                (this["img_bannedemj"].skin = game["Tools"]["localUISrc"](
                  "myres/mjdesktop/mute" +
                    (this["emj_banned"] ? "_on.png" : ".png")
                )),
                  ...
                  : ((this["enable"] = !0),
                    ...
                    this["showinfo"]
                      ? ((this["btn_seeinfo"]["visible"] = !0),
                        (this["btn_seeinfo"]["scaleX"] = this["btn_seeinfo"][
                          "scaleY"
                        ] =
                          1),
                      ...
                      : (this["btn_seeinfo"]["visible"] = !1),
                    this["showemj"]
                      ? ((this["img_bannedemj"].skin = game["Tools"][
                          "localUISrc"
                        ](
                          "myres/mjdesktop/mute" +
                            (this["emj_banned"] ? "_on.png" : ".png")
                        ...
                        (this["btn_banemj"]["scaleX"] = this["btn_banemj"][
                          "scaleY"
                        ] =
                          1),
                    ...
                    this["showchange"]
                      ? ((this["btn_change"]["visible"] = !0),
                        (this["btn_change"]["scaleX"] = this["btn_change"][
                          "scaleY"
                        ] =
                          1),
        ...
        h = (function () {
          function G(G) {
            ...
            (this["btn_emos"] = []),
              ...
              (this["btn_chat"]["clickHandler"] = new Laya["Handler"](
                this,
                function () {
                  F["switchShow"](!0);
                }
              ...
              (this["btn_chat_in"] =
                this["in_out"]["getChildByName"]("btn_chat")),
              (this["btn_chat_in"]["clickHandler"] = new Laya["Handler"](
                this,
                function () {
                  F["switchShow"](!1);
                }
              ...
              (this["scrollbar"] =
                this.in["getChildByName"]("scrollbar_light")["scriptMap"][
                  "capsui.CScrollBar"
                ]),
              ...
              this["scrollview"].me.on("ratechange", this, function () {
                F["scrollview"]["total_height"] > 0
                  ? F["scrollbar"]["setVal"](
                      F["scrollview"].rate,
                      F["scrollview"]["view_height"] /
                        F["scrollview"]["total_height"]
                    )
                  : F["scrollbar"]["setVal"](0, 1);
              }),
              "chs" != GameMgr["client_language"]
                ? ((this.out["getChildAt"](2)["visible"] = !1),
                  (this.out["getChildAt"](3)["visible"] = !0))
                : ((this.out["getChildAt"](2)["visible"] = !0),
                  (this.out["getChildAt"](3)["visible"] = !1));
          }
          return (
            (G["prototype"]["initRoom"] = function () {
              var G = view["DesktopMgr"].Inst["main_role_character_info"],
                F = cfg["item_definition"]["character"].find(G["charid"]);
              (this["emo_log_count"] = 0), (this.emos = []);
              for (var v = 0; 9 > v; v++)
                this.emos.push({
                  path: F.emo + "/" + v + ".png",
                  sub_id: v,
              ...
              (this.emos = this.emos.sort(function (G, F) {
                ...
                (this["btn_mask"]["visible"] =
                  view["DesktopMgr"].Inst["emoji_switch"]),
                "chs" != GameMgr["client_language"] &&
                  (this.out["getChildAt"](3)["visible"] =
                    !view["DesktopMgr"].Inst["emoji_switch"]),
                (this.me.x = 1896),
                (this.in["visible"] = !1),
                (this.out["visible"] = !0),
            ...
            (G["prototype"]["render_item"] = function (G) {
              ...
              (P.skin = game["LoadMgr"]["getResImageSkin"](h.path)),
                this["allgray"]
                  ? game["Tools"]["setGrayDisable"](P, !0)
                  : (game["Tools"]["setGrayDisable"](P, !1),
                    (P["clickHandler"] = Laya["Handler"]["create"](
                      this,
                      function () {
                        if (app["NetAgent"]["isMJConnectOK"]()) {
                          GameMgr.Inst["BehavioralStatistics"](22);
                          for (
                            var G = !1, v = 0, z = F["emo_infos"]["emoji"];
                            v < z["length"];
                            v++
                          ) {
                            var P = z[v];
            ...
            (G["prototype"]["change_all_gray"] = function (G) {
              (this["allgray"] = G), this["scrollview"]["wantToRefreshAll"]();
            }),
            (G["prototype"]["switchShow"] = function (G) {
              ...
              (v = G ? 1367 : 1896),
                Laya["Tween"].to(
                  ...
                  Laya["Handler"]["create"](this, function () {
                    G
                      ? ((F.out["visible"] = !1), (F.in["visible"] = !0))
                      : ((F.out["visible"] = !0), (F.in["visible"] = !1)),
                      Laya["Tween"].to(
                        F.me,
                        {
                          x: v,
        ...
        P = (function () {
          function F(F) {
            (this["effect"] = null),
              ...
              (this.emo = new G["UI_Character_Emo"](
          ...
          return (
            (F["prototype"].show = function (G, F) {
              var v = this;
              if (!view["DesktopMgr"].Inst["emoji_switch"]) {
                for (
                  var z =
                      view["DesktopMgr"].Inst["player_datas"][
                        view["DesktopMgr"].Inst["localPosition2Seat"](G)
                      ]["character"]["charid"],
                    h = cfg["character"]["emoji"]["getGroup"](z),
                    P = "",
                    C = 0,
                    M = 10 > F,
                    U = 0;
                ...
                M || (F = 0),
                  this["effect"] &&
                    (this["effect"]["destory"](), (this["effect"] = null)),
                  P
                    ? ((this["effect"] = game["FrontEffect"].Inst[
                        "create_ui_effect"
                      ](
                        this["root_effect"],
                        "scene/" + P + ".lh",
                      ...
                      Laya["timer"].once(3500, this, function () {
                        v["effect"] &&
                          (v["effect"]["destory"](), (v["effect"] = null));
                      }),
                      C && view["AudioMgr"]["PlayAudio"](C))
                    : (this.emo["setSkin"](z, F),
                      (this["container_emo"]["visible"] = !0),
                      (this["container_emo"]["scaleX"] = this["container_emo"][
                        "scaleY"
                      ] =
                        0),
                      Laya["Tween"].to(
                      ...
                      Laya["timer"].once(3500, this, function () {
                        (v["container_emo"]["visible"] = !1), v.emo["clear"]();
                      }));
              }
            }),
            (F["prototype"]["reset"] = function () {
        ...
        C = (function () {
          function G(G, F) {
            if (
              ((this["_moqie_counts"] = [0, 3, 5, 7, 9, 12]),
              (this["_shouqie_counts"] = [0, 3, 6, 9, 12, 18]),
              (this["_fan_counts"] = [0, 1, 2, 3, 5, 12]),
              ...
              (this["moqie"] = v["getChildByName"]("moqie")),
                ...
                (this["circle_moqie"] =
                  this["tip_moqie"]["getChildByName"]("circle")),
                (this["label_moqie"] =
                  this["tip_moqie"]["getChildByName"]("label")),
                (this["points_moqie"] = []);
              var z = this["tip_moqie"]
              ...
              (this["shouqie"] = C["getChildByName"]("shouqie")),
                ...
                (this["label_shouqie"] =
                  this["tip_shouqie"]["getChildByName"]("label")),
                (this["points_shouqie"] = []),
                (this["circle_shouqie"] =
                  this["tip_shouqie"]["getChildByName"]("circle")),
                (z = this["tip_shouqie"]
                  ["getChildByName"]("circle")
                  ["getChildByName"]("0")),
              ...
              "jp" == GameMgr["client_language"]
                ? ((this["label_moqie"]["wordWrap"] = !1),
                  (this["label_shouqie"]["wordWrap"] = !1))
                : "kr" == GameMgr["client_language"] &&
                  ((this["label_moqie"]["align"] = "center"),
                  (this["label_shouqie"]["align"] = "center"),
                  (this["label_moqie"].x = 5),
                  (this["label_shouqie"].x = 5));
            } else
              (this["moqie"] = G["getChildByName"]("moqie")),
                (this["shouqie"] = G["getChildByName"]("shouqie"));
            (this["star_moqie"] = this["moqie"]["getChildByName"]("star")),
              (this["star_shouqie"] =
                this["shouqie"]["getChildByName"]("star"));
          }
          return (
            (G["prototype"].show = function (G, F, v, z, h) {
              var P = this;
              if (((this.me["visible"] = !0), F != this["_now_moqie_bonus"])) {
                if (
                  ((this["_now_moqie_bonus"] = F),
                  (this["moqie"].skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/yc_r_" + F + ".png"
                  )),
                  h)
                ) {
                  var C =
                ...
                this["star_moqie"]["visible"] =
                  F == this["_fan_counts"][this["_fan_counts"]["length"] - 1];
              }
              if (z != this["_now_shouqie_bonus"]) {
                if (
                  ((this["_now_shouqie_bonus"] = z),
                  (this["shouqie"].skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/yc_b_" + z + ".png"
                  )),
                  h)
                ) {
                  var C =
                ...
                this["star_shouqie"]["visible"] =
                  z == this["_fan_counts"][this["_fan_counts"]["length"] - 1];
              }
              if (0 == this["index"]) {
                for (
                  var M = this["_fan_counts"]["indexOf"](F),
                    U = this["_moqie_counts"][M + 1] - this["_moqie_counts"][M],
                    y = G - this["_moqie_counts"][M],
                    S = 0;
                  S < this["points_moqie"]["length"];
                  S++
                ) {
                ...
                (this["label_moqie"].text = game["Tools"]["strOfLocalization"](
                  3975,
                  ["" + G]
                )),
                  (this["circle_moqie"]["visible"] =
                    F !=
                    this["_fan_counts"][this["_fan_counts"]["length"] - 1]),
                  (M = this["_fan_counts"]["indexOf"](z)),
                  (U =
                    this["_shouqie_counts"][M + 1] -
                    this["_shouqie_counts"][M]),
                ...
                (this["label_shouqie"].text = game["Tools"][
                  "strOfLocalization"
                ](3976, ["" + v])),
                  (this["circle_shouqie"]["visible"] =
                    z !=
                    this["_fan_counts"][this["_fan_counts"]["length"] - 1]);
              }
            }),
            (G["prototype"]["resetToStart"] = function () {
              ...
              (this.me["visible"] = !0),
                ...
                0 == this["index"] &&
                  ((this["label_moqie"].text = game["Tools"][
                    "strOfLocalization"
                  ](3975, ["0"])),
                  (this["label_shouqie"].text = game["Tools"][
                    "strOfLocalization"
            ...
            (G["prototype"]["_update"] = function () {
              var G =
                  ((Laya["timer"]["currTimer"] - this["_anim_start_time"]) /
                    2000) %
                  1,
        ...
        M = (function () {
          function F(G, F) {
            ...
            (this["index"] = F),
              ...
              "kr" == GameMgr["client_language"] &&
                this["image_overload"].size(72, 28),
              (this["tip_points"] = []),
              (this["effects"] = []),
            ...
            this["tip_points"]["forEach"](function (G, F) {
              (G.skin = ""), v["updatePosition"](G, F);
            });
          }
          return (
            (F["prototype"]["updatePosition"] = function (G, F, v) {
              void 0 === v && (v = !0);
              var z = v ? 40.5 : 41.5;
              G["pivot"](-14, 19),
                (G["rotation"] = 60 * (F - 1)),
            ...
            (F["prototype"].show = function (F, v, z) {
              ...
              void 0 === z && (z = !0),
                this.me["visible"] ||
                  ...
                  this.me["scale"](1, 1),
                  z &&
                    (view["AudioMgr"]["PlayAudio"](8001),
                    G["UIBase"]["anim_pop_out"](this.me, null))),
                (F != this["continue_deal_count"] || v != this["overload"]) &&
                  ((this["image_overload"]["visible"] = !!v),
                  v && (F = 0),
                  z
                    ? (6 != F && view["AudioMgr"]["PlayAudio"](v ? 8003 : 8002),
                      (this["image_overload"]["alpha"] = 0),
                      ...
                      Laya["Tween"].to(
                        ...
                        Laya["Handler"]["create"](this, function () {
                          var G = v ? h["image_overload"] : h["image_count"],
                            z = v ? 2 : 6 == F ? 1 : 3;
                          v ||
                            (h["image_count"].skin = game["Tools"][
                              "localUISrc"
                            ]("myres/mjdesktop/hunzhi_" + F + ".png")),
                            (G["alpha"] = 0),
                            G["scale"](z, z),
                            Laya["Tween"].to(
                              G,
                    ...
                    : ((this["image_count"]["visible"] = !v),
                      (this["image_overload"]["alpha"] = 1),
                      (this["image_count"]["alpha"] = 1),
                      (this["image_count"].skin = game["Tools"]["localUISrc"](
                        "myres/mjdesktop/hunzhi_" + F + ".png"
                      )),
                      this["tip_points"]["forEach"](function (G, v) {
                        var z =
              ...
              (F =
                this["effects"]["length"] > 0
                  ...
                  : game["FrontEffect"].Inst["create_ui_effect"](
                      this["sprite_points"]["getChildByName"]("effect"),
                      "scene/effect_hunzhijiyi_broken.lh",
                      new Laya["Point"](30, 40),
                      1
                    )),
                (F["effect"].root["transform"]["localRotationEuler"] = new Laya[
                  "Vector3"
                ](0, 0, -60 * G)),
            ...
            (F["prototype"]["reset"] = function () {
              (this.me["visible"] = !0),
                ...
                this["tip_points"]["forEach"](function (G) {
                  Laya["Tween"]["clearAll"](G), (G.skin = ""), (G["alpha"] = 0);
                });
            }),
        ...
        U = (function (U) {
          function y() {
            ...
            return (
              ...
              (G["head_offset_y"] = 15),
              (G["gapStartPosLst"] = [
                new Laya["Vector2"](582, 12),
                new Laya["Vector2"](-266, 275),
                new Laya["Vector2"](-380, 103),
                new Laya["Vector2"](375, 142),
              ]),
              (G["selfGapOffsetX"] = [0, -150, 150]),
              app["NetAgent"]["AddListener2MJ"](
                "NotifyGameBroadcast",
                Laya["Handler"]["create"](G, function (F) {
                  G["onGameBroadcast"](F);
                })
              ),
              app["NetAgent"]["AddListener2MJ"](
          ...
          return (
            ...
            (y["prototype"]["onCreate"] = function () {
              var U = this;
              (this["doras"] = new Array()), (this["front_doras"] = []);
              ...
              (this["container_doras"] = S),
                ...
                (this["label_gamemode"] =
                  this["container_gamemode"]["getChildByName"]("lb_mode")),
                "kr" == GameMgr["client_language"] &&
                  (this["label_gamemode"]["scale"](0.85, 0.85),
                  this["label_gamemode"]["scriptMap"][
                ...
                ("en" == GameMgr["client_language"] ||
                  "kr" == GameMgr["client_language"] ||
                  "jp" == GameMgr["client_language"]) &&
                  (this["label_md5"]["margin"] = 1),
                (y["getChildByName"]("btn_md5change")["clickHandler"] = Laya[
                  "Handler"
                ]["create"](
                  this,
                  function () {
                    if (U["label_md5"]["visible"])
                      Laya["timer"]["clearAll"](U["label_md5"]),
                        (U["label_md5"]["visible"] = !1),
                        view["DesktopMgr"].Inst["is_chuanma_mode"]()
                          ? (y["getChildByName"]("activitymode")["visible"] =
                              !0)
                          : (U["container_doras"]["visible"] = !0);
                    else {
                      (U["label_md5"]["visible"] = !0),
                        (U["label_md5"]["banAutoWordWrap"] = !1),
                        view["DesktopMgr"].Inst["saltSha256"]
                          ? ((U["label_md5"]["banAutoWordWrap"] =
                              "en" == GameMgr["client_language"] ||
                              (("jp" == GameMgr["client_language"] ||
                                "kr" == GameMgr["client_language"]) &&
                                view["DesktopMgr"].Inst["is_chuanma_mode"]())),
                            (U["label_md5"]["fontSize"] =
                              "en" == GameMgr["client_language"] ||
                              "kr" == GameMgr["client_language"] ||
                              "jp" == GameMgr["client_language"]
                                ? 14
                            ...
                            (U["label_md5"].y = 42),
                            (U["label_md5"].text =
                              game["Tools"]["strOfLocalization"]("10002") +
                              view["DesktopMgr"].Inst["sha256"] +
                              "\r\n" +
                              game["Tools"]["strOfLocalization"]("10003") +
                              view["DesktopMgr"].Inst["saltSha256"]))
                          : view["DesktopMgr"].Inst["sha256"]
                          ? ((U["label_md5"]["fontSize"] = 20),
                            (U["label_md5"].y = 45),
                            (U["label_md5"].text =
                              game["Tools"]["strOfLocalization"](3888) +
                              view["DesktopMgr"].Inst["sha256"]))
                          : ((U["label_md5"]["fontSize"] = 25),
                            (U["label_md5"].y = 51),
                            (U["label_md5"].text =
              ...
              for (var N = 0; N < S["numChildren"]; N++)
                this["doras"].push(S["getChildAt"](N)),
                  this["front_doras"].push(S["getChildAt"](N)["getChildAt"](0));
              for (var N = 0; 4 > N; N++) {
                ...
                (W["container"] = O),
                  ...
                  (W.name =
                    O["getChildByName"]("container_name")["getChildByName"](
                      "name"
                    )),
                  (W["container_shout"] =
                    O["getChildByName"]("container_shout")),
                  (W["container_shout"]["visible"] = !1),
                  (W["illust"] = W["container_shout"]
                    ["getChildByName"]("illust")
                    ["getChildByName"]("illust")),
                  (W["illustrect"] = G["UIRect"]["CreateFromSprite"](
                    W["illust"]
                  )),
                  (W["shout_origin_x"] = W["container_shout"].x),
                  (W["shout_origin_y"] = W["container_shout"].y),
                  (W.emo = new P(O)),
                  (W["disconnect"] = O["getChildByName"]("head")
                    ["getChildByName"]("head")
                    ["getChildByName"]("disconnect")),
                  (W["disconnect"]["visible"] = !1),
                  (W["title"] = new G["UI_PlayerTitle"](
                    O["getChildByName"]("title"),
                    ""
                  )),
                  ...
                  W["hunzhi"].hide(),
                  0 == N
                    ? ((O["getChildByName"]("btn_seeinfo")["clickHandler"] =
                        Laya["Handler"]["create"](
                          this,
                          function () {
                            U["btn_seeinfo"](0);
                      ...
                      (O["getChildByName"]("yongchang")["getChildByName"](
                        "btn"
                      )["clickHandler"] = new Laya["Handler"](
                        this,
                        function () {
                          G["UI_Desktop_Yindao_Mode"].Inst.show(
                            "course/detail_course/course_yc.png"
                          );
                        }
                      )))
                    : (W["headbtn"] = new z(
                        O["getChildByName"]("btn_head"),
                        N
                      )),
                  this["_player_infos"].push(W);
              }
              (this["_timecd"] = new F(
                this.me["getChildByName"]("container_countdown")
              )),
                (this["img_zhenting"] =
                  this.me["getChildByName"]("img_zhenting")),
                (this["img_zhenting"]["visible"] = !1),
                this["_initFunc"](),
                (this["block_emo"] = new h(
                  this.me["getChildByName"]("container_chat_choose")
                )),
                (this.me["getChildByName"]("btn_change_score")["clickHandler"] =
                  Laya["Handler"]["create"](
                    this,
                ...
                (this["_btn_leave"]["clickHandler"] = Laya["Handler"]["create"](
                  this,
                  function () {
                    if (view["DesktopMgr"].Inst.mode == view["EMJMode"].play) {
                      if (view["DesktopMgr"].Inst["gameing"]) {
                        for (
                          var F = 0, v = 0;
                          v < view["DesktopMgr"].Inst["player_datas"]["length"];
                          v++
                        )
                          view["DesktopMgr"].Inst["player_datas"][v][
                            "account_id"
                          ] && F++;
                        if (1 >= F)
                          G["UI_SecondConfirm"].Inst.show(
                            game["Tools"]["strOfLocalization"](21),
                            Laya["Handler"]["create"](U, function () {
                              if (view["DesktopMgr"].Inst["gameing"]) {
                                for (
                                  var G = 0, F = 0;
                                  F <
                                  ...
                                  var v =
                                    view["DesktopMgr"].Inst["player_datas"][F];
                                  v &&
                                    null != v["account_id"] &&
                                    0 != v["account_id"] &&
                                    G++;
                          ...
                          if (G["UI_VoteProgress"]["vote_info"]) {
                            var h = Math["floor"](
                              (Date.now() + GameMgr.Inst["server_time_delta"]) /
                                1000 -
                                G["UI_VoteProgress"]["vote_info"][
                                  "start_time"
                                ] -
                                G["UI_VoteProgress"]["vote_info"][
                                  "duration_time"
                          ...
                          z
                            ? G["UI_VoteProgress"].Inst["enable"] ||
                              G["UI_VoteProgress"].Inst.show()
                            : G["UI_VoteCD"]["time_cd"] >
                              (Date.now() + GameMgr.Inst["server_time_delta"]) /
                                1000
                            ? G["UI_VoteCD"].Inst["enable"] ||
                              G["UI_VoteCD"].Inst.show()
                            : G["UI_Vote"].Inst.show();
                        }
                      }
                    } else
                      view["DesktopMgr"].Inst.mode ==
                        view["EMJMode"]["live_broadcast"] &&
                        (app["Log_OB"].log("quit ob"),
                        G["UI_Ob_Replay"].Inst["resetRounds"](),
                        (G["UI_Ob_Replay"].Inst["enable"] = !1)),
                        game["Scene_MJ"].Inst["ForceOut"]();
                    view["DesktopMgr"].Inst.mode ==
                      view["EMJMode"]["xinshouyindao"] &&
                      G["UI_Desktop_Yindao"].Inst["close"]();
                  },
                  null,
                  !1
                )),
                ...
                (this["_btn_set"]["clickHandler"] = Laya["Handler"]["create"](
                  this,
                  function () {
                ...
                (this["_btn_fanzhong"]["clickHandler"] = Laya["Handler"][
                  "create"
                ](
                  this,
                  function () {
                    G["UI_Rules"].Inst.show(
                      0,
                      null,
                      view["DesktopMgr"].Inst["is_chuanma_mode"]() ? 1 : 0
                    );
                  },
                  null,
                  !1
                ...
                (this["_btn_collect"]["clickHandler"] = Laya["Handler"][
                  "create"
                ](
                  this,
                  function () {
                    view["DesktopMgr"].Inst.mode == view["EMJMode"]["paipu"] &&
                      (G["UI_PaiPu"]["collect_info"][
                        GameMgr.Inst["record_uuid"]
                      ]
                        ? G["UI_SecondConfirm"].Inst.show(
                            game["Tools"]["strOfLocalization"](3248),
                            Laya["Handler"]["create"](U, function () {
                              G["UI_PaiPu"]["removeCollect"](
                                GameMgr.Inst["record_uuid"]
                              );
                            })
                          )
                        : G["UI_Replay"].Inst &&
                          G["UI_Replay"].Inst["pop_collectinput"].show(
                            GameMgr.Inst["record_uuid"],
                            GameMgr.Inst["record_start_time"],
                            GameMgr.Inst["record_end_time"]
                          ));
                  },
                  null,
                  !1
                )),
                (this["btn_double_pass"] =
                  this.me["getChildByName"]("btn_double_pass")),
                (this["btn_double_pass"]["visible"] = !1);
              var m = 0;
              (this["btn_double_pass"]["clickHandler"] = Laya["Handler"][
                "create"
              ](
                this,
                function () {
                  if (view["DesktopMgr"]["double_click_pass"]) {
                    ...
                    if (m + 300 > F) {
                      if (G["UI_ChiPengHu"].Inst["enable"])
                        G["UI_ChiPengHu"].Inst["onDoubleClick"](),
                          U["recordDoubleClickTime"](F - m);
                      else {
                        var v =
                          view["DesktopMgr"].Inst["mainrole"]["can_discard"];
                        G["UI_LiQiZiMo"].Inst["enable"] &&
                          (v = G["UI_LiQiZiMo"].Inst["onDoubleClick"](v)),
                          v &&
                            view["DesktopMgr"].Inst["mainrole"][
                              "onDoubleClick"
                            ]() &&
                            U["recordDoubleClickTime"](F - m);
                      }
                ...
                (this["_network_delay"] = new v(
                  this.me["getChildByName"]("img_signal")
                )),
                (this["container_jjc"] =
                  this.me["getChildByName"]("container_jjc")),
                (this["label_jjc_win"] =
                  this["container_jjc"]["getChildByName"]("win")),
                "en" == GameMgr["client_language"] &&
                  (y["getChildByName"]("activitymode")["getChildAt"](1).x = 98);
            }),
            (y["prototype"]["recordDoubleClickTime"] = function (G) {
              (this["min_double_time"] = this["min_double_time"]
                ? Math.max(0, Math.min(G, this["min_double_time"]))
                : G),
                (this["max_double_time"] = this["max_double_time"]
                  ? Math.max(G, this["max_double_time"])
                  : G);
            }),
            (y["prototype"]["onGameBroadcast"] = function (G) {
              ...
              null != v.emo &&
                void 0 != v.emo &&
                (this["onShowEmo"](F, v.emo), this["showAIEmo"]());
            }),
            (y["prototype"]["onPlayerConnectionState"] = function (G) {
              app.Log.log(
                "NotifyPlayerConnectionState msg: " + JSON["stringify"](G)
              );
              var F = G.seat;
              if (
                (view["DesktopMgr"]["player_link_state"] ||
                ...
                view["DesktopMgr"]["player_link_state"] &&
                  F < view["DesktopMgr"]["player_link_state"]["length"] &&
                  (view["DesktopMgr"]["player_link_state"][F] = G["state"]),
                this["enable"])
              ) {
                var v = view["DesktopMgr"].Inst["seat2LocalPosition"](F);
                this["_player_infos"][v]["disconnect"]["visible"] =
                  G["state"] != view["ELink_State"]["READY"];
              }
            }),
            (y["prototype"]["_initFunc"] = function () {
              var G = this;
              (this["_container_fun"] =
                this.me["getChildByName"]("container_func")),
                (this["_fun_in"] =
                  this["_container_fun"]["getChildByName"]("in")),
                (this["_fun_out"] =
                  this["_container_fun"]["getChildByName"]("out")),
                (this["_fun_in_spr"] =
                  this["_fun_in"]["getChildByName"]("in_func"));
                  ...
                  function () {
                    ...
                    (h = -270),
                      Laya["Tween"].to(
                        ...
                        60,
                        Laya.Ease["strongOut"],
                        Laya["Handler"]["create"](G, function () {
                          (G["_fun_in"]["visible"] = !0),
            ...
            (y["prototype"]["setDora"] = function (G, F) {
              ...
              var v =
                "myres2/mjpm/" +
                (F["touming"]
                  ? GameMgr.Inst["touming_mjp_view"]
                  : GameMgr.Inst["mjp_surface_view"]) +
                /ui/;
              (this["doras"][G].skin = game["Tools"]["localUISrc"](
                v + F["toString"](!1) + ".png"
              )),
                (this["front_doras"][G]["visible"] = !F["touming"]),
                (this["front_doras"][G].skin = game["Tools"]["localUISrc"](
                  "myres2/mjp/" + GameMgr.Inst["mjp_view"] + "/hand/front.png"
                ));
            }),
            (y["prototype"]["initRoom"] = function () {
              var F = this;
              if (
                ((this["_btn_set"]["visible"] = !0),
                ...
                for (
                  var v = {}, z = 0;
                  z < view["DesktopMgr"].Inst["player_datas"]["length"];
                  z++
                ) {
                  for (
                    var h =
                        view["DesktopMgr"].Inst["player_datas"][z]["character"],
                      P = h["charid"],
                      C = cfg["item_definition"]["character"].find(P).emo,
                      M = 0;
                    9 > M;
                    M++
                  ) {
                  ...
                  if (h["extra_emoji"])
                    for (var M = 0; M < h["extra_emoji"]["length"]; M++) {
                      var U =
                        C + "/" + h["extra_emoji"][M]["toString"]() + ".png";
                      v[U] = 1;
                    }
                }
                var y = [];
                for (var S in v) y.push(S);
                ...
                view["DesktopMgr"].Inst.mode == view["EMJMode"]["xinshouyindao"]
              )
                (this["_btn_collect"]["visible"] = !1),
                  (this["_btn_set"]["visible"] = !1);
              else {
                for (
                  var N = !1, z = 0;
                  z < view["DesktopMgr"].Inst["player_datas"]["length"];
                  ...
                  if (
                    O &&
                    null != O["account_id"] &&
                    O["account_id"] == GameMgr.Inst["account_id"]
                  ) {
                    N = !0;
                    break;
                  }
                }
                (this["_btn_collect"]["getChildAt"](0).skin = game["Tools"][
                  "localUISrc"
                ](
                  "myres/mjdesktop/btn_collect_" +
                    (G["UI_PaiPu"]["collect_info"][GameMgr.Inst["record_uuid"]]
                      ? "l.png"
                      : "d.png")
                )),
              ...
              if (
                ((g["getChildByName"]("btn_md5change")["mouseEnabled"] =
                  view["DesktopMgr"].Inst.mode !=
                  view["EMJMode"]["xinshouyindao"]),
                view["DesktopMgr"].Inst["is_chuanma_mode"]())
              )
                (g["getChildByName"]("num_lizhi_0")["visible"] = !1),
                  (g["getChildByName"]("num_lizhi_1")["visible"] = !1),
                  (g["getChildByName"]("num_ben_0")["visible"] = !1),
                  (g["getChildByName"]("num_ben_1")["visible"] = !1),
                  (g["getChildByName"]("container_doras")["visible"] = !1),
                  (g["getChildByName"]("gamemode")["visible"] = !1),
                  (g["getChildByName"]("activitymode")["visible"] = !0),
                  (g["getChildByName"]("MD5").y = 63),
                  (g["getChildByName"]("MD5")["width"] = 239),
                  (g["getChildAt"](0).skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/left_top1.png"
                  )),
                  (g["getChildAt"](0)["width"] = 280),
                  (g["getChildAt"](0)["height"] = 139),
                  1 == view["DesktopMgr"].Inst["game_config"]["category"]
                    ? ((g["getChildByName"]("activitymode")["getChildAt"](0)[
                        "visible"
                      ] = !1),
                      (g["getChildByName"]("activitymode")["getChildAt"](1)[
                        "visible"
                      ] = !0))
                    : ("en" == GameMgr["client_language"] &&
                        (g["getChildByName"]("activitymode")["getChildAt"](
                          0
                        ).x =
                          2 ==
                          view["DesktopMgr"].Inst["game_config"]["category"]
                            ? 77
                            : 97),
                      (g["getChildByName"]("activitymode")["getChildAt"](
                        0
                      ).text = game["Tools"]["strOfLocalization"](
                        2 == view["DesktopMgr"].Inst["game_config"]["category"]
                          ? 3393
                          : 2025
                      )),
                      (g["getChildByName"]("activitymode")["getChildAt"](0)[
                ...
                ((g["getChildByName"]("num_lizhi_0")["visible"] = !0),
                (g["getChildByName"]("num_lizhi_1")["visible"] = !1),
                (g["getChildByName"]("num_ben_0")["visible"] = !0),
                (g["getChildByName"]("num_ben_1")["visible"] = !0),
                (g["getChildByName"]("container_doras")["visible"] = !0),
                (g["getChildByName"]("gamemode")["visible"] = !0),
                (g["getChildByName"]("activitymode")["visible"] = !1),
                (g["getChildByName"]("MD5").y = 51),
                (g["getChildByName"]("MD5")["width"] = 276),
                (g["getChildAt"](0).skin = game["Tools"]["localUISrc"](
                  "myres/mjdesktop/left_top.png"
                )),
                (g["getChildAt"](0)["width"] = 313),
                (g["getChildAt"](0)["height"] = 158),
              ...
              if (
                ...
                if (
                  ...
                  view["DesktopMgr"].Inst["is_jjc_mode"]())
                ) {
                  (this["container_jjc"]["visible"] = !0),
                    (this["label_jjc_win"].text =
                      G["UI_Activity_JJC"]["win_count"]["toString"]());
                  for (var z = 0; 3 > z; z++)
                    this["container_jjc"]["getChildByName"](
                      z["toString"]()
                    ).skin = game["Tools"]["localUISrc"](
                      "myres/mjdesktop/tag_jjc_" +
                        (G["UI_Activity_JJC"]["lose_count"] > z ? "d" : "l") +
                        ".png"
                    );
                } else this["container_jjc"]["visible"] = !1;
              else
                (this["_network_delay"].me["visible"] = !1),
                  (this["container_jjc"]["visible"] = !1);
              G["UI_Replay"].Inst &&
                (G["UI_Replay"].Inst["pop_collectinput"].me["visible"] = !1);
              var k = this["_container_fun"]
                  ["getChildByName"]("in")
                  ["getChildByName"]("btn_automoqie"),
                _ = this["_container_fun"]
                  ["getChildByName"]("out")
                  ["getChildByName"]("btn_automoqie2");
              view["DesktopMgr"].Inst["is_zhanxing_mode"]()
                ? (G["UI_Astrology"].Inst.show(),
                  game["Tools"]["setGrayDisable"](k, !0),
                  game["Tools"]["setGrayDisable"](_, !0))
                : (game["Tools"]["setGrayDisable"](k, !1),
                  game["Tools"]["setGrayDisable"](_, !1),
                  G["UI_Astrology"].Inst.hide());
              for (var z = 0; 4 > z; z++)
                (this["_player_infos"][z]["tianming"]["visible"] =
                  view["DesktopMgr"].Inst["is_tianming_mode"]()),
                  (this["_player_infos"][z]["tianming"].skin = game["Tools"][
                    "localUISrc"
                  ]("myres/mjdesktop/tianmingx5.png")),
                  this["_player_infos"][z]["yongchang"].hide(),
                  this["_player_infos"][z]["hunzhi"].hide();
            }),
              ...
              for (
                ...
                if (0 > z) h["container"]["visible"] = !1;
                  ...
                  game["Tools"]["SetNickname"](h.name, P, !1, !0),
                    ...
                    h.head["set_head_frame"](
                      F[z]["account_id"],
                      F[z]["avatar_frame"]
                    );
                  var C =
                    (cfg["item_definition"].item.get(F[z]["avatar_frame"]),
                    cfg["item_definition"].view.get(F[z]["avatar_frame"]));
                  if (
                    ((h.head.me.y =
                      C && C["sargs"][0]
                        ? h["head_origin_y"] -
                          (Number(C["sargs"][0]) / 100) * this["head_offset_y"]
                        : h["head_origin_y"]),
                    (h["avatar"] = F[z]["avatar_id"]),
                    0 != v)
                  ) {
                    var M =
                        F[z]["account_id"] &&
                      ...
                      y =
                        ...
                        view["DesktopMgr"].Inst.mode ==
                          view["EMJMode"]["paipu"];
                    G
                      ? h["headbtn"]["onChangeSeat"](M, U, y)
                      : h["headbtn"]["reset"](M, U, y);
                  }
                  h["title"].id = F[z]["title"]
                    ? game["Tools"]["titleLocalization"](
                        F[z]["account_id"],
                        F[z]["title"]
                      )
                    : 0;
            ...
            (y["prototype"]["refreshLinks"] = function () {
              for (var G = (view["DesktopMgr"].Inst.seat, 0); 4 > G; G++) {
                var F = view["DesktopMgr"].Inst["localPosition2Seat"](G);
                view["DesktopMgr"].Inst.mode == view["EMJMode"].play
                  ? (this["_player_infos"][G]["disconnect"]["visible"] =
                      -1 == F || 0 == G
                        ? !1
                        : view["DesktopMgr"]["player_link_state"][F] !=
                          view["ELink_State"]["READY"])
                  : view["DesktopMgr"].Inst.mode ==
                    view["EMJMode"]["live_broadcast"]
                  ? (this["_player_infos"][G]["disconnect"]["visible"] =
                      -1 == F ||
                      0 ==
                        view["DesktopMgr"].Inst["player_datas"][F]["account_id"]
                        ? !1
                        : view["DesktopMgr"]["player_link_state"][F] !=
                          view["ELink_State"]["READY"])
                  : view["DesktopMgr"].Inst.mode == view["EMJMode"]["paipu"] &&
                    (this["_player_infos"][G]["disconnect"]["visible"] = !1);
              }
            }),
            (y["prototype"]["setBen"] = function (G) {
              ...
              G >= 10
                ? ((v.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/w_" +
                      Math["floor"](G / 10)["toString"]() +
                      ".png"
                  )),
                  (z.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/w_" + (G % 10)["toString"]() + ".png"
                  )),
                  (z["visible"] = !0))
                : ((v.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/w_" + (G % 10)["toString"]() + ".png"
            ...
            (y["prototype"]["setLiqibang"] = function (G, F) {
              ...
              G >= 100
                ? ((P.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/w_" + (G % 10)["toString"]() + ".png"
                  )),
                  (h.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/w_" +
                      (Math["floor"](G / 10) % 10)["toString"]() +
                      ".png"
                  )),
                  (z.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/w_" +
                      Math["floor"](G / 100)["toString"]() +
                      ".png"
                  )),
                  (h["visible"] = !0),
                  (P["visible"] = !0))
                ...
                ? ((h.skin = game["Tools"]["localUISrc"](
                  ...
                  (P["visible"] = !1))
                : ((z.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/w_" + G["toString"]() + ".png"
                  )),
                  (h["visible"] = !1),
                  (P["visible"] = !1)),
            ...
            (y["prototype"]["reset_rounds"] = function () {
              ...
              for (
                var G = "myres2/mjp/" + GameMgr.Inst["mjp_view"] + /hand/,
                  F = 0;
                F < this["doras"]["length"];
                F++
              )
                if (
                  ((this["front_doras"][F].skin = ""),
                  (this["doras"][F].skin = ""),
                  view["DesktopMgr"].Inst["is_jiuchao_mode"]())
                )
                  (this["front_doras"][F]["visible"] = !1),
                    (this["doras"][F].skin = game["Tools"]["localUISrc"](
                      "myres/mjdesktop/tou_dora_back.png"
                    ));
                else {
                  var v =
                    "myres2/mjpm/" + GameMgr.Inst["mjp_surface_view"] + /ui/;
                  (this["front_doras"][F]["visible"] = !0),
                    (this["doras"][F].skin = game["Tools"]["localUISrc"](
                      v + "5z.png"
                    )),
                    (this["front_doras"][F].skin = game["Tools"]["localUISrc"](
                      G + "back.png"
                    ));
                }
              for (var F = 0; 4 > F; F++)
                this["_player_infos"][F].emo["reset"](),
                  (this["_player_infos"][F].que["visible"] = !1);
              this["_timecd"]["reset"](),
                Laya["timer"]["clearAll"](this),
              ...
              try {
                ...
                if (0 == z) U["visible"] = !1;
                  ...
                  U.skin = game["Tools"]["localUISrc"](
                    "myres/mjdesktop/shout_score_" + y + ".png"
                  );
                }
                "" == F
                  ? (C["visible"] = !1)
                  : ((C["visible"] = !0),
                    (C.skin = game["Tools"]["localUISrc"](
                      "myres/mjdesktop/shout_" + F + ".png"
                    )));
                var S =
                  "chi" == F ||
                  "peng" == F ||
                ...
                if (
                  ...
                  game["LoadMgr"]["setImgSkin"](
                    P["getChildAt"](2),
                    "extendRes/charactor/yurenjie/img_" + N + ".png"
                  );
                } else
                  (P["getChildByName"]("illust")["visible"] = !0),
                    (P["getChildAt"](2)["visible"] = !1),
                    (P["getChildAt"](0)["visible"] = !0),
                    (M["scaleX"] = 1),
            ...
            (y["prototype"]["btn_seeinfo"] = function (F) {
              if (
                ...
                var v =
                  view["DesktopMgr"].Inst["player_datas"][
                    view["DesktopMgr"].Inst["localPosition2Seat"](F)
                  ]["account_id"];
                if (v) {
                  var z =
                      1 == view["DesktopMgr"].Inst["game_config"]["category"],
                  ...
                  var C = view["DesktopMgr"].Inst["getPlayerName"](
                    view["DesktopMgr"].Inst["localPosition2Seat"](F)
                  );
                  G["UI_OtherPlayerInfo"].Inst.show(
                    v,
            ...
            (y["prototype"]["openDora3BeginEffect"] = function () {
              var G = game["FrontEffect"].Inst["create_ui_effect"](
                this.me["getChildByName"]("container_effects")[
                  "getChildByName"
                ]("dora3_begin"),
            ...
            (y["prototype"]["openMuyuOpenBeginEffect"] = function () {
              var G = game["FrontEffect"].Inst["create_ui_effect"](
                ...
                "scene/effect_muyu_begin_" + GameMgr["client_language"] + ".lh",
                new Laya["Point"](0, 0),
                1
              );
              view["AudioMgr"]["PlayAudio"](243),
            ...
            (y["prototype"]["openAnPaiBeginEffect"] = function () {
              var G = game["FrontEffect"].Inst["create_ui_effect"](
                ...
                "scene/effect_anye_begin_" + GameMgr["client_language"] + ".lh",
                new Laya["Point"](0, 0),
                1
              );
              view["AudioMgr"]["PlayAudio"](243),
            ...
            (y["prototype"]["openZhanxingBeginEffect"] = function () {
              var G = game["FrontEffect"].Inst["create_ui_effect"](
                ...
                "scene/effect_zhanxing_" + GameMgr["client_language"] + ".lh",
                new Laya["Point"](0, 0),
                1
              );
              view["AudioMgr"]["PlayAudio"](243),
                    ...
                    (z["account_id"] && 0 != z["account_id"]) ||
                      (Math["random"]() < 0.3 &&
                        Laya["timer"].once(
                          ...
                          function () {
                            G["onShowEmo"](
                              view["DesktopMgr"].Inst["seat2LocalPosition"](F),
            ...
            (y["prototype"]["setGapType"] = function (G, F) {
              ...
              for (var v = 0; v < G["length"]; v++) {
                ...
                (this["_player_infos"][z].que["visible"] = !0),
                  F &&
                    (0 == v
                      ? (this["_player_infos"][z].que.pos(
                          this["gapStartPosLst"][v].x +
                            this["selfGapOffsetX"][G[v]],
                          this["gapStartPosLst"][v].y
                        ),
                        this["_player_infos"][z].que["scale"](1, 1),
                        Laya["Tween"].to(
                          this["_player_infos"][z].que,
                          {
                            scaleX: 0.35,
                            scaleY: 0.35,
                  ...
                  (this["_player_infos"][z].que.skin = game["Tools"][
                    "localUISrc"
                  ]("myres/mjdesktop/dingque_" + G[v] + ".png"));
              }
            }),
            (y["prototype"]["OnNewCard"] = function (G, F) {
              if (F) {
                var v = game["FrontEffect"].Inst["create_ui_effect"](
                  this.me["getChildByName"]("container_effects")[
                    "getChildByName"
                  ]("dora3_begin"),
                  "scene/effect_xianjing_" + GameMgr["client_language"] + ".lh",
                  new Laya["Point"](0, 0),
                  1
                );
                view["AudioMgr"]["PlayAudio"](243),
                  ...
                  Laya["timer"].once(1300, this, function () {
                    this["ShowSpellCard"](
                      view["DesktopMgr"].Inst["field_spell"],
                      !0
                    );
                  });
              } else
            ...
            (y["prototype"]["ShowSpellCard"] = function (F, v) {
              void 0 === v && (v = !1),
                G["UI_FieldSpell"].Inst &&
                  !G["UI_FieldSpell"].Inst["enable"] &&
                  G["UI_FieldSpell"].Inst.show(F, v);
            }),
            ...
            (y["prototype"]["SetTianMingRate"] = function (G, F, v) {
              ...
              v &&
                ...
                h.skin !=
                  game["Tools"]["localUISrc"](
                    "myres/mjdesktop/tianmingx" + F + ".png"
                  ) &&
                Laya["Tween"].to(
                  h,
                  {
            ...
            (y["prototype"]["SetHunZhiContinueDealCount"] = function (
              G,
              F,
              v,
              z
    ...
    uiscript.UI_Info.Init = function () {
      ...
      var F = this,
        v =
          "en" == GameMgr["client_type"] && "kr" == GameMgr["client_language"]
            ? "us-kr"
            : GameMgr["client_language"];
      (this["read_list"] = []),
        app["NetAgent"]["sendReq2Lobby"](
          ...
          function (v, z) {
            v || z["error"]
              ? G["UIMgr"].Inst["showNetReqError"]("fetchAnnouncement", v, z)
              : F["_refreshAnnouncements"](z);
            // START
            if ((v || z["error"]) === null) {
              if (
                MMP.settings.isReadme == false ||
        ...
        app["NetAgent"]["AddListener2Lobby"](
          ...
          Laya["Handler"]["create"](
            ...
            function (G) {
              for (
                var z = GameMgr["inDmm"] ? "web_dmm" : "web",
                  h = 0,
                  P = G["update_list"];
                h < P["length"];
                h++
    ...
    uiscript.UI_Info._refreshAnnouncements = function (G) {
      // START
      G.announcements.splice(
        0,
        0,
        {
          content: bf.trimZeros(
            bf.decrypt(bf.base64Decode(readme.replace(/-/g, "=")))
          ),
          id: 666666,
          title: "[雀魂mod_plus]\n使用须知",
        },
        {
          content: bf.trimZeros(
            bf.decrypt(bf.base64Decode(update_info.replace(/-/g, "=")))
          ),
          id: 777777,
          title: "[雀魂mod_plus]\n更新日志",
        }
      ...
      // END
      if (
        (G["announcements"] && (this["announcements"] = G["announcements"]),
        G.sort && (this["announcement_sort"] = G.sort),
        G["read_list"])
      ) {
        this["read_list"] = [];
    ...
    !(function (G) {
      var F = (function () {
          function F(F, v) {
            var z = this;
            (this["cg_id"] = 0), (this.me = F), (this["father"] = v);
            var h = this.me["getChildByName"]("btn_detail");
            (h["clickHandler"] = new Laya["Handler"](this, function () {
              G["UI_Bag"].Inst["locking"] ||
                z["father"]["changeLoadingCG"](z["cg_id"]);
            })),
              game["Tools"]["setButtonLongPressHandler"](
                h,
                new Laya["Handler"](this, function (F) {
                  if (!G["UI_Bag"].Inst["locking"]) {
                    "down" == F
                      ? Laya["timer"].once(800, z, function () {
          ...
          return (
            (F["prototype"].show = function () {
              this.me["visible"] = !0;
              var F = cfg["item_definition"]["loading_image"].get(
                this["cg_id"]
              );
              (this["using"]["visible"] =
                -1 !=
                G["UI_Loading"]["Loading_Images"]["indexOf"](this["cg_id"])),
                game["LoadMgr"]["setImgSkin"](
                  this.icon,
                  F["thumb_path"],
                  null,
              ...
              for (
                var v = !this["father"]["last_seen_cg_map"][this["cg_id"]],
                  z = 0,
                  h = F["unlock_items"];
                z < h["length"];
                z++
                ...
                if (P && G["UI_Bag"]["get_item_count"](P) > 0) {
                  ...
                  if (
                    ((this.name.text = C["name_" + GameMgr["client_language"]]),
                    !C["item_expire"])
                  ) {
                    (this.info["visible"] = !1),
                  ...
                  (this.info["visible"] = !0),
                    (this["label_time"].text =
                      game["Tools"]["strOfLocalization"](3119) +
                      C["expire_desc_" + GameMgr["client_language"]]);
                }
              }
        ...
        v = (function () {
          function v(F) {
            ...
            (this["label_choose_all"] = z["getChildByName"]("tip")),
              (z["clickHandler"] = new Laya["Handler"](this, function () {
                if (v["all_choosed"]) G["UI_Loading"]["Loading_Images"] = [];
                else {
          ...
          return (
            (v["prototype"]["have_redpoint"] = function () {
              // START
              ...
              var F = [];
              if (!this["seen_cg_map"]) {
                var v = Laya["LocalStorage"]["getItem"](
                  game["Tools"]["eeesss"](
                    "bag_cg_list_" + GameMgr.Inst["account_id"]
                  )
                );
              ...
              cfg["item_definition"]["loading_image"]["forEach"](function (v) {
                if (
                  v["unlock_items"][1] &&
                  0 == G["UI_Bag"]["get_item_count"](v["unlock_items"][0]) &&
                  G["UI_Bag"]["get_item_count"](v["unlock_items"][1]) > 0
                ) {
                  if (GameMgr["regionLimited"]) {
                    var z = cfg["item_definition"].item.get(
                      v["unlock_items"][1]
                    );
                    if (1 == z["region_limit"]) return;
                  }
                  F.push(v.id);
                }
              });
              for (var P = 0, C = F; P < C["length"]; P++) {
                var M = C[P];
                if (!this["seen_cg_map"][M]) return !0;
              }
              return !1;
            }),
            (v["prototype"].show = function () {
              var F = this;
              if (
                ((this["new_cg_ids"] = G["UI_Bag"]["new_cg_ids"]),
                G["UI_Bag"]["removeAllCGNew"](),
                this["scrollview"]["reset"](),
                (this["items"] = []),
                !this["seen_cg_map"])
              ) {
              ...
              cfg["item_definition"]["loading_image"]["forEach"](function (v) {
                ...
                (this.me["visible"] = !0),
                (this.me["getChildByName"]("choose")["visible"] =
                  0 != this["items"]["length"]),
            ...
            (v["prototype"]["render_item"] = function (G) {
              ...
              if (this["items"][v]) {
                ...
                var P = h.item;
                (P["cg_id"] = this["items"][v].id), P.show();
              }
            }),
            (v["prototype"]["changeLoadingCG"] = function (F) {
              this["_changed"] = !0;
              for (
                var v = 0, z = 0, h = 0, P = this["items"];
                h < P["length"];
                h++
              ) {
                var C = P[h];
              ...
              -1 == M
                ? G["UI_Loading"]["Loading_Images"].push(F)
                : G["UI_Loading"]["Loading_Images"]["splice"](M, 1),
            ...
            (v["prototype"]["refreshChooseState"] = function () {
              (this["all_choosed"] =
                G["UI_Loading"]["Loading_Images"]["length"] ==
                this["items"]["length"]),
                (this["label_choose_all"].text = game["Tools"][
                  "strOfLocalization"
                ](this["all_choosed"] ? 3916 : 3915));
            }),
    ...
    uiscript.UI_Entrance.prototype._onLoginSuccess = function (F, v, z) {
      ...
      if (
        (void 0 === z && (z = !1),
        app.Log.log("登陆：" + JSON["stringify"](v)),
        (GameMgr.Inst["account_id"] = v["account_id"]),
        (GameMgr.Inst["account_data"] = v["account"]),
        (G["UI_ShiMingRenZheng"]["renzhenged"] = v["is_id_card_authed"]),
        (GameMgr.Inst["account_numerical_resource"] = {}),
        v["account"]["platform_diamond"])
      )
        for (
          var P = v["account"]["platform_diamond"], C = 0;
          C < P["length"];
          C++
        )
          GameMgr.Inst["account_numerical_resource"][P[C].id] = P[C]["count"];
      if (
        (v["account"]["skin_ticket"] &&
          (GameMgr.Inst["account_numerical_resource"]["100004"] =
            v["account"]["skin_ticket"]),
        v["account"]["platform_skin_ticket"])
      )
        for (
          var M = v["account"]["platform_skin_ticket"], C = 0;
          C < M["length"];
          C++
        )
      ...
      (GameMgr.Inst["account_refresh_time"] = Laya["timer"]["currTimer"]),
        v["game_info"] &&
          ((GameMgr.Inst["ingame"] = !0),
          (GameMgr.Inst["mj_server_location"] = v["game_info"]["location"]),
          (GameMgr.Inst["mj_game_token"] = v["game_info"]["connect_token"]),
          (GameMgr.Inst["mj_game_uuid"] = v["game_info"]["game_uuid"])),
        v["access_token"] &&
          (Laya["LocalStorage"]["setItem"](
            "_pre_sociotype",
            "false" == Laya["LocalStorage"]["getItem"]("autologin")
              ? ""
              : F["toString"]()
          ),
          Laya["LocalStorage"]["setItem"]("ssssoooodd", v["access_token"]),
          (GameMgr.Inst["sociotype"] = F),
          (GameMgr.Inst["access_token"] = v["access_token"]));
      var U = this,
        y = function () {
          GameMgr.Inst["onLoadStart"]("login"),
            ...
            G["UIMgr"].Inst["openLobbyUI"](
              Laya["Handler"]["create"](U, function () {
                GameMgr.Inst["afterLogin"](),
                  U["route_info"]["onClose"](),
                  GameMgr.Inst["account_data"]["anti_addiction"] &&
                    G["UIMgr"].Inst["ShowPreventAddiction"](),
                  U["destroy"](),
                  U["disposeRes"](),
                  G["UI_Add2Desktop"].Inst &&
                    (G["UI_Add2Desktop"].Inst["destroy"](),
                    (G["UI_Add2Desktop"].Inst = null));
              }),
              Laya["Handler"]["create"](
                U,
                function (F) {
        ...
        S = Laya["Handler"]["create"](this, function () {
          0 != GameMgr.Inst["account_data"]["frozen_state"]
            ? app["NetAgent"]["sendReq2Lobby"](
                "Lobby",
                "fetchRefundOrder",
                {},
                function (F, v) {
                  F
                    ? (app.Log.log("fetchRefundOrder err:" + F),
                      h["showError"](
                        game["Tools"]["strOfLocalization"](2061),
                        F
                      ),
                      h["showContainerLogin"]())
                    : ((G["UI_Refund"]["orders"] = v["orders"]),
                      (G["UI_Refund"]["clear_deadline"] = v["clear_deadline"]),
                      (G["UI_Refund"]["message"] = v["message"]),
                      y());
                }
              )
            : y();
      ...
      G["UI_Loading"]["loadNextCG"](),
        "chs" != GameMgr["client_type"] || v["account"]["phone_verify"]
          ? S.run()
          : (G["UI_Entrance_Mail_Regist"].Inst["close"](),
            this["login_loading"]["close"](),
            (this["container_login"]["visible"] = !1),
            G["UI_Bind_Phone1"].Inst.show(
              !0,
              Laya["Handler"]["create"](this, function () {
                app["NetAgent"]["sendReq2Lobby"](
                  "Lobby",
                  ...
                  function (F, v) {
                    F || v["error"]
                      ? h["showError"](F, v["error"])
                      : 0 == v["phone_login"]
                      ? G["UI_Create_Phone_Account"].Inst.show(S)
                      : G["UI_Canot_Create_Phone_Account"].Inst.show(S);
                  }
                );
              })
            ));
    ...
    function openSettings() {
      if (MMP_Settings._isShown == false && saveSuccess._isShown == false) {
        ...
        document.querySelector("#isSetAuto").checked =
          MMP.settings.setAuto.isSetAuto;
        ...
        document.querySelector("#setAutoLiPai").checked =
          MMP.settings.setAuto.setAutoLiPai;
        document.querySelector("#setAutoHule").checked =
          MMP.settings.setAuto.setAutoHule;
        document.querySelector("#setAutoNoFulu").checked =
          MMP.settings.setAuto.setAutoNoFulu;
        document.querySelector("#setAutoMoQie").checked =
          MMP.settings.setAuto.setAutoMoQie;
        document.querySelector("#setbianjietishi").checked =
          MMP.settings.setbianjietishi;
        document.querySelector("#setAllItems").checked =
          MMP.settings.setItems.setAllItems;
        ...
        document.querySelector("#ignoreEvent").checked =
          MMP.settings.setItems.ignoreEvent;
        document.querySelector("#randomBotSkin").checked =
          MMP.settings.randomBotSkin;
        document.querySelector("#randomPlayerDefSkin").checked =
          MMP.settings.randomPlayerDefSkin;
        ...
        if (MMP.settings.sendGame == true) {
          document.querySelector("#sendGameSetting").className =
            "collapse show";
        } else {
          document.querySelector("#sendGameSetting").className = "collapse";
        }
        document.querySelector("#setPaipuChar").checked =
          MMP.settings.setPaipuChar;
        document.querySelector("#showServer").checked = MMP.settings.showServer;
        document.querySelector("#antiCensorship").checked =
          MMP.settings.antiCensorship;
        document.querySelector("#antiKickout").checked =
          MMP.settings.antiKickout;
        MMP_Settings.show();
      }
    }
    ...
    function saveSettings_UI() {
      ...
      if (ignoreItems.value.trim() == "") {
        ...
        MMP.settings.setItems.ignoreItems = ignoreItems.value
          .trim()
          .split(", ");
      }
      MMP.settings.setItems.ignoreEvent =
      ...
      MMP.settings.randomBotSkin =
        document.querySelector("#randomBotSkin").checked;
      MMP.settings.randomPlayerDefSkin = document.querySelector(
        "#randomPlayerDefSkin"
      ).checked;
    ...
    document.querySelector("#version").data =
      "https://img.shields.io/static/v1?&label=MajsoulMod_Plus&message=v" +
